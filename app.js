/* GERADO por build.js a partir de app.jsx — não edite à mão. */
const {
  useState,
  useMemo,
  useEffect
...
const CHECKOUT = {
  gestao: {
    mensal: 'https://pay.kiwify.com.br/BCrSogz',
    anual: 'https://pay.kiwify.com.br/VS7YGUR'
  },
  pro: {
    mensal: 'https://pay.kiwify.com.br/rRaBx79',
    anual: 'https://pay.kiwify.com.br/QcBjiXF'
  }
};
...
const PLAN_LABEL = {
  ...
  pro: 'Pro (R$ 49,90/mês)',
  founder: 'Fundador (tudo liberado)',
  team: 'Time'
};
// tour guiado do 1º acesso: navega tela a tela com um cartão explicando cada parte
const TOUR_STEPS = [{
  v: 'painel',
  t: 'Painel — a foto geral',
  x: 'Sua banca atual, o resultado da semana e o lucro acumulado. É a primeira tela de todo dia.'
}, {
  v: 'torneios',
  t: 'Torneios — a fonte de tudo',
  x: 'Lança cada torneio aqui (buy-in, posição, prêmio). Diário, Semanal e a banca se atualizam sozinhos a partir disso.'
}, {
  v: 'diario',
  t: 'Diário — o dia consolidado',
  x: 'Quantos torneios, ABI médio e resultado de cada dia — com alerta de stop loss quando o dia passa do limite que você definiu.'
}, {
  v: 'semanal',
  t: 'Semanal — o fechamento',
  x: 'Resultado semana a semana e a régua do ABI: a disciplina que separa reg de degen.'
}, {
  v: 'banca',
  t: 'Banca — o dinheiro',
  x: 'Saldos separados por sala (adicione as salas que você joga nos Ajustes), aportes, retiradas e transferências.'
}, {
  v: 'stats',
  t: 'Stats — suas estatísticas',
  x: 'Importa suas mãos (zip do PokerCraft da GG ou .txt do PokerStars) e vê VPIP, bb/100 por posição, a sorte nos all-ins e as leituras automáticas em português.'
}, {
  v: 'config',
  t: 'Ajustes — suas regras',
  x: 'Seu nome, grade de buy-in, stop loss, salas e seu plano. Toda mudança fica registrada no histórico. Fim do tour — bom grind! 🃏'
}];
...
function downloadCSV(name, rows) {
  ...
  a.href = URL.createObjectURL(new Blob(['\uFEFF' + toCSV(rows)], {
...
const abiMaxFor = (config, player, date) => {
  ...
  if (date) {
    ...
    for (const c of hist) {
      if (String(c.created_at).slice(0, 10) > date && num(c.old_value) > 0) v = num(c.old_value);
    }
  }
  return v;
};
...
function computeWeeks(daily, config) {
  ...
  const makeInit = {
    [config.player1_name]: num(config.makeup_inicial_player1),
    [config.player2_name]: num(config.makeup_inicial_player2)
  };
  const out = {};
  players.forEach(p => {
...
const STATUS_TONE = {
  ...
  'Bloqueado por make-up': {
    tone: C.red,
    bg: C.redSoft
  },
  'Bloqueado por banca abaixo do piso': {
    tone: C.red,
    bg: C.redSoft
  }
};
...
/* ============================================================
   ...
   ============================================================ */
const HH_RANKS = '23456789TJQKA',
  HH_SUITS = 'cdhs';
const hhCard = s => {
  const r = HH_RANKS.indexOf((s || '')[0]),
    u = HH_SUITS.indexOf(((s || '')[1] || '').toLowerCase());
...
function hhScore(cs) {
  const R = c => c >> 2,
    S = c => c & 3,
    byR = Array(13).fill(0);
  cs.forEach(c => byR[R(c)]++);
  const uniq = [];
  for (let r = 12; r >= 0; r--) if (byR[r]) uniq.push(r);
  const stTop = rs => {
    const set = new Set(rs);
    if (set.has(12)) set.add(-1);
    for (let h = 12; h >= 3; h--) {
      let ok = true;
      for (let k = 0; k < 5; k++) if (!set.has(h - k)) {
        ok = false;
        break;
      }
      if (ok) return h;
  ...
  const sc = (cat, ts) => {
    let v = cat;
    for (let i = 0; i < 5; i++) v = v * 15 + (ts && ts[i] != null ? ts[i] + 1 : 0);
    return v;
  };
  for (let s = 0; s < 4; s++) {
    const fr = cs.filter(c => S(c) === s).map(R).sort((a, b) => b - a);
    if (fr.length >= 5) {
      const st = stTop([...new Set(fr)]);
      return st >= 0 ? sc(9, [st]) : sc(6, fr.slice(0, 5));
    }
  }
  const quad = uniq.find(r => byR[r] === 4);
  if (quad != null) return sc(8, [quad, uniq.find(r => r !== quad)]);
  const trips = uniq.filter(r => byR[r] === 3),
    pairs = uniq.filter(r => byR[r] === 2);
  if (trips.length && (trips.length > 1 || pairs.length)) return sc(7, [trips[0], trips.length > 1 ? trips[1] : pairs[0]]);
  const st = stTop(uniq);
  if (st >= 0) return sc(5, [st]);
...
function hhEquity(hero, vils, board) {
  ...
  let win = 0,
    tot = 0;
  const run = ex => {
    const full = board.concat(ex);
    const hs = hhScore(hero.concat(full));
    let best = hs,
      n = 1,
      heroTop = true;
    for (const v of vils) {
      const vs = hhScore(v.concat(full));
      if (vs > best) {
        best = vs;
        heroTop = false;
        n = 1;
  ...
  if (need <= 0) run([]);else if (need === 1) deck.forEach(c => run([c]));else if (need === 2) {
    ...
    for (let k = 0; k < 1500; k++) {
      const d = deck.slice(),
        ex = [];
      for (let m = 0; m < need; m++) ex.push(d.splice(Math.floor(rnd() * d.length), 1)[0]);
      run(ex);
    }
  }
...
function hhParseHand(block, why) {
  ...
  const mT = head.match(/(?:Tournament|Torneio) #([^,\s]+)/);
  if (!mT) return skip('cash'); // só torneios
  const tid = mT[1];
  // blinds: PS "(35/70)" ou "(50/100/10)" · GG "Level6(150/300(35))" com ante aninhado
  const mBB = head.match(/\(([\d,]+)\/([\d,]+)(?:[\/(]([\d,]+)\)?)?\)/);
  const bb = mBB ? hhNum(mBB[2]) : 0;
  if (!bb) return skip('formato');
  const mD = head.match(/(\d{4})[\/-](\d{2})[\/-](\d{2})/);
  const date = mD ? `${mD[1]}-${mD[2]}-${mD[3]}` : null;
  const mH = head.match(/#([^:\s]+):/);
  const hid = mH ? mH[1] : null; // nº da mão (dedup)
  const seg = head.slice(0, Math.max(0, head.search(/Hold'em/)));
  let buyin = 0;
  (seg.match(/\$\s?[\d.,]+/g) || []).forEach(x => buyin += hhNum(x));
  ...
  for (let i = hi; i < lines.length; i++) {
    const L = lines[i];
    let m;
    if (/\*\*\* (SUMMARY|SUMÁRIO)/.test(L)) {
      inSummary = true;
      continue;
    } // às vezes vem colado no fim de outra linha
    if (inSummary) continue;
    if (m = L.match(/(?:Seat|Lugar) #(\d+) (?:is the button|é o botão)/)) {
      button = +m[1];
      continue;
    }
    if (m = L.match(/^(?:Seat|Lugar) (\d+): (.+?) \(\$?\s?([\d,.]+) (?:in chips|em fichas)/)) {
      seats.push({
        seat: +m[1],
        n: m[2],
        chips: hhNum(m[3])
    ...
    if (m = L.match(/^Dealt to (.+?) \[([^\]]+)\]/)) {
      hero = m[1];
      heroCards = m[2].split(/\s+/).map(hhCard);
      continue;
    }
    if (m = L.match(/^(.+?) recebe \[([^\]]+)\]/)) {
      hero = m[1];
    ...
    if (/^\*\*\* FLOP/.test(L)) {
      ...
      commit = {};
      const mm = L.match(/\[([^\]]+)\]/);
      if (mm) mm[1].split(/\s+/).forEach(c => {
        const k = hhCard(c);
        if (k >= 0 && board.length < 3) board.push(k);
      });
      continue;
    }
    if (/^\*\*\* TURN/.test(L)) {
    ...
    if (/^\*\*\* SHOW ?DOWN/.test(L)) {
      showdown = true;
      continue;
    } // PS "SHOW DOWN" · GG "SHOWDOWN"
    if (m = L.match(/^(?:Uncalled bet|Aposta não-igualada) \(?\$?\s?([\d,.]+)\)?\s+(?:returned to|voltou para) (.+)$/)) {
      addInv(m[2], -hhNum(m[1]));
      continue;
    }
    if (m = L.match(/^(.+?) collected \$?([\d,]+) from/)) {
      collected[m[1]] = (collected[m[1]] || 0) + hhNum(m[2]);
      continue;
    }
    if (m = L.match(/^(.+?) recebeu \$?\s?([\d,.]+) do(?: \S+)? pote/)) {
      collected[m[1]] = (collected[m[1]] || 0) + hhNum(m[2]);
      continue;
    } // PT: pote / principal pote / secundário pote
    if (m = L.match(/^(.+?): (?:shows|mostra) \[([^\]]+)\]/)) {
      shows[m[1]] = m[2].split(/\s+/).map(hhCard).filter(c => c >= 0);
      continue;
    }
    if (m = L.match(/^(.+?): (?:posts (?:the )?ante|coloca ante) \$?\s?([\d,.]+)/)) {
      addInv(m[1], hhNum(m[2]));
      continue;
    }
    if (m = L.match(/^(.+?): (?:posts|paga o) (?:small|big) blind \$?\s?([\d,.]+)/)) {
      addInv(m[1], hhNum(m[2]));
      commit[m[1]] = (commit[m[1]] || 0) + hhNum(m[2]);
      continue;
    }
    if (m = L.match(/^(.+?): (?:posts|paga)\b.*?([\d,.]+)$/)) {
      addInv(m[1], hhNum(m[2]));
      continue;
    } // straddle/dead blind
    if (m = L.match(/^(.+?): (folds|checks|calls|bets|raises|desiste|passa|iguala|aposta|aumenta)\b\s*\$?\s?([\d,.]*)(?:\s+(?:to|para)\s+\$?\s?([\d,.]+))?/)) {
      const n = m[1],
        act = ACT_PT[m[2]] || m[2],
        allin = /and is all[- ]in|e está all-in/.test(L);
      if (act === 'calls' || act === 'bets') {
        const x = hhNum(m[3]);
        addInv(n, x);
        commit[n] = (commit[n] || 0) + x;
      }
      if (act === 'raises') {
        const to = hhNum(m[4] || m[3]);
        addInv(n, Math.max(0, to - (commit[n] || 0)));
        commit[n] = to;
      }
      acts.push({
        n,
  ...
  const heroInv = invested[hero] || 0,
    heroCol = collected[hero] || 0,
    net = heroCol - heroInv;
  // posições (relativas ao botão)
  const order = seats.slice().sort((a, b) => a.seat - b.seat);
  let bi = order.findIndex(s => s.seat === button);
  if (bi < 0) bi = 0;
  const nP = order.length,
    after = i => order[(bi + i) % nP].n;
  const posMap = {};
  if (nP === 2) {
    posMap[after(0)] = 'BTN';
    posMap[after(1)] = 'BB';
    ...
    for (let i = 3; i < nP; i++) {
      const d = nP - i;
      posMap[after(i)] = d === 1 ? 'CO' : d === 2 ? 'MP' : 'EP';
    }
  }
  ...
  for (const a of pre) {
    if (a.n === hero) {
      ...
      if (!heroActedPre) {
        heroActedPre = true;
        if (nP >= 3 && raises === 0 && callersBefore === 0 && foldedToHero && ['CO', 'BTN', 'SB'].includes(heroPos)) {
          stealOpp = true;
          if (a.act === 'raises') steal = true;
        }
        if (raises === 1 && lastRaiser !== hero) {
          tbOpp = true;
          if (a.act === 'raises') tb = true;
        }
        if (nP >= 3 && heroPos === 'BB' && raises === 1 && ['CO', 'BTN', 'SB'].includes(posMap[firstRaiser]) && callersBefore === 0) {
          bbdefOpp = true;
          if (a.act === 'calls' || a.act === 'raises') bbdef = true;
        }
      }
    ...
    if (a.act === 'raises') {
      raises++;
      lastRaiser = a.n;
      if (!firstRaiser) firstRaiser = a.n;
      if (a.n === hero) heroRaised = true;else if (heroRaised) pendingF3b = true;
    }
  }
  ...
  if (sawflop && raises > 0) {
    for (const a of acts.filter(x => x.street === 1)) {
      if (a.n === hero) {
        if (lastRaiser === hero && !betSeen && !cbetOpp) {
          cbetOpp = true;
          cbet = a.act === 'bets';
        }
        if (lastRaiser && lastRaiser !== hero && betSeen && betBy === lastRaiser && !fcbOpp) {
          fcbOpp = true;
          fcb = a.act === 'folds';
        }
      }
      if (a.act === 'bets' && !betSeen) {
        betSeen = true;
        betBy = a.n;
      }
    }
  ...
  const wtsd = showdown && !heroFolded && Object.keys(shows).length > 0,
    wsd = wtsd && heroCol > 0,
    wwsf = sawflop && heroCol > 0;
  let afB = 0,
    afC = 0;
  acts.forEach(a => {
    if (a.n === hero && a.street > 0) {
      if (a.act === 'bets' || a.act === 'raises') afB++;
      if (a.act === 'calls') afC++;
    }
  });
...
function parseHH(text) {
  ...
  const blocks = text.split(/(?=^(?:PokerStars Hand|PokerStars Game|Mão PokerStars|Poker Hand|GGPoker Hand) #)/m).filter(b => /^(?:PokerStars Hand|PokerStars Game|Mão PokerStars|Poker Hand|GGPoker Hand) #/.test(b.trim()));
  for (const b of blocks) {
    const why = {
      r: null
    };
    try {
      const h = hhParseHand(b.trim(), why);
      if (h) hands.push(h);else {
        ignored++;
        const r = why.r || 'formato';
        reasons[r] = (reasons[r] || 0) + 1;
      }
    } catch (e) {
      ignored++;
      reasons.formato = (reasons.formato || 0) + 1;
      if (issues.length < 10) issues.push('mão ignorada: ' + e.message);
    }
  }
  return {
    hands,
...
function hhAggregate(hands, player) {
  ...
  for (const h of hands) {
    ...
    if (!agg[k]) agg[k] = {
      player,
      site: h.site,
      site_tournament_id: h.tid,
      tournament_name: h.tname,
      entry_date: h.date,
      buyin: h.buyin,
      hands: 0,
      net_bb: 0,
    ...
    a.hands++;
    a.net_bb += h.net_bb;
    if (h.vpip) a.vpip_cnt++;
    if (h.pfr) a.pfr_cnt++;
    if (h.tbOpp) a.tb_opp++;
    if (h.tb) a.tb_cnt++;
    if (h.f3bOpp) a.f3b_opp++;
    if (h.f3b) a.f3b_cnt++;
    if (h.stealOpp) a.steal_opp++;
    if (h.steal) a.steal_cnt++;
    if (h.bbdefOpp) a.bbdef_opp++;
    if (h.bbdef) a.bbdef_cnt++;
    if (h.cbetOpp) a.cbet_opp++;
    if (h.cbet) a.cbet_cnt++;
    if (h.fcbOpp) a.fcbet_opp++;
    if (h.fcb) a.fcbet_cnt++;
    if (h.sawflop) a.sawflop_cnt++;
    if (h.wwsf) a.wwsf_cnt++;
    if (h.wtsd) a.wtsd_cnt++;
    if (h.wsd) a.wsd_cnt++;
    a.af_bets += h.afB;
    a.af_calls += h.afC;
    if (h.allin) {
      a.allin_cnt++;
      a.allin_ev_bb += h.allin.ev;
      a.allin_net_bb += h.allin.net;
    }
    ...
    if (!a.pos_json[h.pos]) a.pos_json[h.pos] = {
      h: 0,
      v: 0,
      p: 0,
      net: 0,
    ...
    a.pos_json[h.pos].h++;
    if (h.vpip) a.pos_json[h.pos].v++;
    if (h.pfr) a.pos_json[h.pos].p++;
    a.pos_json[h.pos].net += h.net_bb;
    a.pos_json[h.pos].hn++;
    if (!a.stack_json[h.bucket]) a.stack_json[h.bucket] = {
      h: 0,
      v: 0,
      p: 0,
      net: 0
    };
    const sb2 = a.stack_json[h.bucket];
    sb2.h++;
    if (h.vpip) sb2.v++;
    if (h.pfr) sb2.p++;
    sb2.net += h.net_bb;
  ...
  return Object.values(agg).map(a => {
    const pj = {};
    Object.keys(a.pos_json).forEach(k => {
      pj[k] = {
        ...a.pos_json[k],
        net: Math.round(a.pos_json[k].net * 100) / 100
      };
    });
    return {
      ...a,
      pos_json: pj,
      net_bb: Math.round(a.net_bb * 100) / 100,
      allin_ev_bb: Math.round(a.allin_ev_bb * 100) / 100,
      allin_net_bb: Math.round(a.allin_net_bb * 100) / 100
    };
  });
}
// soma um agregado de mãos NOVAS numa linha já existente do banco (import incremental sem contar 2x)
const HH_SUM_KEYS = ['hands', 'net_bb', 'vpip_cnt', 'pfr_cnt', 'tb_cnt', 'tb_opp', 'f3b_cnt', 'f3b_opp', 'steal_cnt', 'steal_opp', 'bbdef_cnt', 'bbdef_opp', 'cbet_cnt', 'cbet_opp', 'fcbet_cnt', 'fcbet_opp', 'sawflop_cnt', 'wwsf_cnt', 'wtsd_cnt', 'wsd_cnt', 'af_bets', 'af_calls', 'allin_cnt', 'allin_ev_bb', 'allin_net_bb'];
function mergeHH(a, b) {
  ...
  const mj = (x, y) => {
    ...
    Object.keys(y || {}).forEach(k => {
      if (!o[k]) o[k] = {
        ...y[k]
      };else Object.keys(y[k]).forEach(kk => o[k][kk] = num(o[k][kk]) + num(y[k][kk]));
    });
    return o;
  };
  out.pos_json = mj(a.pos_json, b.pos_json);
  out.stack_json = mj(a.stack_json, b.stack_json);
...
const HH_GLOSS = [['VPIP', '% de mãos em que você pôs ficha por vontade própria antes do flop (call ou raise; blind obrigatório não conta). Mede quantas mãos você joga.'], ['PFR', '% de mãos em que você deu raise pré-flop. Quanto mais perto do VPIP, mais agressivo (e melhor) o estilo.'], ['3-bet', 'Re-raise em cima de quem abriu. Pouco 3-bet = os outros abrem de graça contra você.'], ['Fold pra 3-bet', 'Quantas vezes você desistiu depois de abrir e levar um re-raise. Muito baixo = paga demais; muito alto = vira alvo.'], ['Roubo de blinds', 'Abertura de CO, BTN ou SB quando todos foldaram antes — a jogada mais lucrativa do MTT com antes.'], ['BB defende', 'Quantas vezes você (no BB) pagou ou re-raisou contra um roubo, em vez de foldar.'], ['C-bet flop', 'Aposta de continuação: você abriu pré-flop e seguiu apostando no flop.'], ['Fold pra c-bet', 'Quantas vezes você desistiu no flop contra a c-bet do agressor. Alto demais = "fit-or-fold", fácil de explorar.'], ['WWSF', '% das vezes que você viu o flop e LEVOU o pote, com ou sem showdown (potes ganhos ÷ flops vistos). Mede briga pós-flop.'], ['WTSD', '% das vezes que, tendo visto o flop, você foi até o showdown (showdowns ÷ flops vistos — definição padrão de tracker). Atenção: o HUD da GG mostra o "WT" sobre TODAS as mãos, por isso o número de lá é bem menor que o daqui. Alto = paga muito; baixo = desiste muito.'], ['W$SD', '% dos showdowns que você ganhou (showdowns ganhos ÷ showdowns disputados). Lido junto com o WTSD: ir muito ao showdown e ganhar pouco = vazamento clássico.'], ['AF (agressão)', '(apostas + raises) ÷ calls no pós-flop. Abaixo de ~1,5 = passivo; acima de ~4 = hiperagressivo.'], ['bb/100', 'Resultado em big blinds a cada 100 mãos, em FICHAS de torneio — mede qualidade de jogo, não dinheiro direto.'], ['Sorte (all-in EV)', 'Explicada no card "Sorte nos all-ins" acima. Mede só os all-ins com cartas viradas — sorte pré-all-in (cooler, distribuição), bounty de PKO e ICM ficam fora da conta.']];
...
function hhInsights(d) {
  ...
  const p = {
    ...
    tb: pct(d.tb, d.tbOpp),
    f3b: pct(d.f3b, d.f3bOpp),
    steal: pct(d.steal, d.stealOpp),
    bbdef: pct(d.bbdef, d.bbdefOpp),
    cbet: pct(d.cbet, d.cbetOpp),
    fcb: pct(d.fcb, d.fcbOpp),
    wtsd: pct(d.wtsd, d.sawflop),
    wsd: pct(d.wsd, d.wtsd),
    wwsf: pct(d.wwsf, d.sawflop)
  };
  ...
  const pp = k => {
    ...
    if (!x || !(num(x.h) > 0)) return null;
    const h = num(x.h);
    return {
      h,
      v: num(x.v) / h * 100,
      pr: num(x.p) / h * 100,
      gap: (num(x.v) - num(x.p)) / h * 100
    };
  };
  ...
  const nb = ['EP', 'MP', 'CO', 'BTN'].reduce((a, k) => {
    const x = d.pos && d.pos[k];
    if (x) {
      a.h += num(x.h);
      a.v += num(x.v);
  ...
  if (d.sawflop >= 100) {
    if (p.wtsd > 33 && p.wsd != null && p.wsd < 47) add('red', 'Chegando ao showdown atrás', `WTSD ${f(p.wtsd)} com W$SD ${f(p.wsd)} (reg: 24–32% / 48–58%). Você paga até o fim e chega sem a melhor mão. A correção não é virar nit: é chegar ao river com range mais forte e escolher bluff catcher por blocker e sizing — não por "a linha do vilão não faz sentido".`, 'wsd');else if (p.wtsd > 34) add('gold', 'Showdown demais', `WTSD ${f(p.wtsd)} (reg: 24–32%). Confira se os calls de turn/river têm equity de verdade.`, 'wtsd');
  ...
  if (d.hands >= 300 && d.bb100 != null) {
    ...
    if (d.bb100 >= 0 && adj >= 0) add('green', 'Winrate saudável', `${par}. O lucro não depende de rodar bem nos all-ins — é jogo, não moeda.${evNote}`, 'bb100');else if (d.bb100 >= 0 && adj < 0) add('gold', 'Lucro inflado pelos all-ins', `${par}. Nos all-ins medidos você recebeu acima do justo: não suba de grade por esse resultado e revise os spots antes que a variância cobre.${evNote}`, 'bb100');else if (d.bb100 < 0 && adj >= 0) add('info', 'Prejuízo com cara de variância', `${par}. Nos all-ins medidos o jogo gerou valor; o resultado é que ainda não mostrou. Mantém o plano, a grade e o volume.${evNote}`, 'bb100');else add('red', 'Winrate negativo mesmo sem azar', `${par}. Nem o azar dos all-ins explica o prejuízo sozinho — prioridade é atacar os vazamentos apontados acima.${evNote}`, 'bb100');
  }
  ...
  if (d.f3bOpp >= 15 && d.fcbOpp >= 30 && d.sawflop >= 100 && p.f3b < 40 && p.fcb < 35 && p.wtsd > 34) add('red', 'Resiste demais depois de entrar no pote', `Fold pra 3-bet ${f(p.f3b)} + fold pra c-bet ${f(p.fcb)} + WTSD ${f(p.wtsd)}: você entra no pote razoavelmente bem, mas não solta a mão quando o adversário mostra força — antes e depois do flop. A evolução não é abrir menos: é melhorar os pontos de saída (largar contra 3-bet fora de posição, abandonar o float que não melhorou, soltar o bluff catcher sem blocker).`);
  ...
  if (d.cbetOpp >= 30 && d.sawflop >= 100 && p.cbet > 70 && p.wwsf < 42) add('gold', 'Uma bala só', `C-bet ${f(p.cbet)} + WWSF ${f(p.wwsf)}: você atira quase sempre no flop mas raramente leva o pote sem showdown — sinal de barril único que desiste no turn. Planeje a mão inteira antes da primeira aposta.`);
  if (d.bbdefOpp >= 30 && d.fcbOpp >= 30 && p.bbdef > 50 && p.fcb > 60) add('gold', 'Defende o BB e larga no flop', `Defesa de BB ${f(p.bbdef)} + fold pra c-bet ${f(p.fcb)}: pagar pra ver e desistir na primeira aposta é doação em duas parcelas. Ou defenda menos mãos, ou brigue mais nos flops que tocam seu range.`);
  {
    const gap2 = p.vpip != null && p.pfr != null ? p.vpip - p.pfr : null;
    if (d.hands >= 200 && gap2 != null && gap2 > 9 && af != null && d.afB + d.afC >= 60 && af < 1.3) add('red', 'Passivo pré e pós-flop', `VPIP−PFR de ${gap2.toFixed(1).replace('.', ',')} pontos + AF ${af.toFixed(1).replace('.', ',')}: o padrão completo é de jogo de call — entra pagando e segue pagando. É o perfil que mais perde no micro; a correção rende mais que qualquer ajuste fino.`);
  }
  if (d.allinCnt >= 15) {
  ...
  const pb = (k, min) => {
    const x = d.pos && d.pos[k];
    return x && num(x.hn) >= (min || 150) ? num(x.net) / num(x.hn) * 100 : null;
  };
  ...
  if (btn100 != null && co100 != null && sb100 != null && bbp100 != null) {
    const late = num(d.pos.BTN.net) + num(d.pos.CO.net),
      blinds = num(d.pos.SB.net) + num(d.pos.BB.net);
...
/* ---------- ícones ---------- */
const Svg = p => /*#__PURE__*/React.createElement("svg", {
  width: p.s || 20,
  height: p.s || 20,
  viewBox: "0 0 24 24",
  fill: "none",
...
const IcoPanel = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("rect", {
  x: "3",
  y: "3",
  width: "7",
  height: "9",
...
const IcoDaily = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("rect", {
  x: "3",
  y: "4",
  width: "18",
  height: "17",
...
const IcoTrophy = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M7 4h10v4a5 5 0 0 1-10 0V4z"
}), /*#__PURE__*/React.createElement("path", {
  d: "M17 5h3v2a3 3 0 0 1-3 3M7 5H4v2a3 3 0 0 0 3 3"
}), /*#__PURE__*/React.createElement("path", {
  d: "M9 15h6l1 5H8l1-5z"
}));
const IcoShield = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  ...
  d: "M9.2 12l2 2 3.6-4.2"
}));
const IcoWeek = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("rect", {
  x: "3",
  y: "4",
  width: "18",
  height: "17",
...
const IcoMonth = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("rect", {
  x: "3",
  y: "4",
  width: "18",
  height: "17",
  ...
  d: "M3 9h18M8 2v4M16 2v4M7 13h3v3H7z"
}));
const IcoCashOut = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M3 7a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2"
}), /*#__PURE__*/React.createElement("path", {
  d: "M12 15V7m0 0l-3 3m3-3l3 3"
}));
const IcoStack = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("ellipse", {
  cx: "12",
  cy: "6",
  rx: "7",
  ry: "3"
...
const IcoGear = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("circle", {
  cx: "12",
  cy: "12",
  r: "3.2"
}), /*#__PURE__*/React.createElement("path", {
...
const IcoChip = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("circle", {
  cx: "12",
  cy: "12",
  r: "9"
}), /*#__PURE__*/React.createElement("circle", {
...
const IcoTrash = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M4 7h16M9 7V5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2M6 7l1 13a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1l1-13"
}));
const IcoX = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M6 6l12 12M18 6L6 18"
}));
const IcoPencil = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M16 3l5 5L8 21H3v-5L16 3z"
}));
const IcoUp = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M7 17L17 7M7 7h10v10"
}));
const IcoDown = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
...
const IcoLogout = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
...
const IcoMenu = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M4 6h16M4 12h16M4 18h16"
}));
const IcoStats = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M3 20h18"
}), /*#__PURE__*/React.createElement("path", {
  d: "M6 20v-6M11 20V5M16 20v-9M21 20v-4"
}));
const IcoEye = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
...
const IcoEyeOff = p => /*#__PURE__*/React.createElement(Svg, p, /*#__PURE__*/React.createElement("path", {
  d: "M3 3l18 18"
}), /*#__PURE__*/React.createElement("path", {
  d: "M10.6 6.1A9.6 9.6 0 0 1 12 6c6.5 0 10 6 10 6a17 17 0 0 1-3.3 3.9M6.1 6.2A17 17 0 0 0 2 12s3.5 7 10 7a9.3 9.3 0 0 0 4.2-1"
}), /*#__PURE__*/React.createElement("path", {
...
const Card = ({
  ...
  style: {
    ...
    borderRadius: 22,
    boxShadow: '0 1px 2px rgba(25,35,29,.05), 0 12px 32px -20px rgba(25,35,29,.28)',
    ...style
  }
}, children);
function NavBtn({
  ...
  return /*#__PURE__*/React.createElement("button", {
    ...
    style: {
      ...
      minWidth: 0,
      flex: mobile ? '1 1 0' : 'initial',
      justifyContent: mobile ? 'center' : 'flex-start',
      padding: mobile ? '8px 4px' : '12px 15px',
      borderRadius: 14,
      border: 'none',
      cursor: 'pointer',
...
const Brand = ({
  ...
  style: {
    ...
    height: small ? 34 : 38,
    borderRadius: 12,
    background: 'linear-gradient(135deg,#5D4DAF 0%,#4C3E92 55%,#332A69 100%)',
    boxShadow: 'inset 0 1px 0 rgba(255,255,255,.25), 0 4px 12px -6px rgba(76,62,146,.5)',
    display: 'grid',
    placeItems: 'center',
    color: '#fff'
  }
...
}, team ? 'Pool de Poker' : 'GrinderBank'), /*#__PURE__*/React.createElement("div", {
  style: {
    fontSize: 11.5,
    color: C.inkSoft,
    fontWeight: 600,
...
}, team ? 'staking · make-up · saques' : 'seu grind sob controle')));
function Badge({
  text
}) {
  const t = STATUS_TONE[text] || {
    tone: C.inkSoft,
    bg: C.bg
  };
...
function MultiBars({
  ...
  const H = 150,
    half = (H - 24) / 2;
  if (!data.length) return /*#__PURE__*/React.createElement(Empty, null, "Ainda sem dados.");
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
  ...
  }, act === i && d.vals.map((v, j) => /*#__PURE__*/React.createElement("span", {
    key: j,
    style: {
      fontSize: 9.5,
      fontWeight: 800,
  ...
  }, num(v) >= 0 ? '+' : '−', "$", Math.abs(num(v)).toFixed(2).replace('.', ',')))), hasNeg ? /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 4,
      alignItems: 'stretch',
    ...
    const h = Math.abs(num(v)) / max * ((H - 40) / 2);
    return /*#__PURE__*/React.createElement("div", {
      key: j,
      title: fmt(v),
      style: {
  ...
  }, d.vals.map((v, j) => /*#__PURE__*/React.createElement("div", {
    key: j,
    title: fmt(v),
    style: {
      width: '42%',
      maxWidth: 16,
      height: `${Math.abs(num(v)) / max * 100}%`,
      background: series[j].color,
    ...
    style: {
      fontSize: 11.5,
      color: act === i ? C.ink : C.inkSoft,
      fontWeight: act === i ? 800 : 600,
      whiteSpace: 'nowrap'
    }
...
function LineChart({
  ...
  const [act, setAct] = useState(null);
  if (!data.length) return /*#__PURE__*/React.createElement(Empty, null, "Ainda sem movimentos na banca.");
  ...
  const track = (cx, el) => {
    ...
    const fr = Math.max(0, Math.min(1, (cx - r.left) / r.width));
    setAct(Math.round(fr * (n - 1)));
  };
  return /*#__PURE__*/React.createElement("div", {
    style: {
      userSelect: 'none'
  ...
  }, ticks.map((tv, k) => /*#__PURE__*/React.createElement("div", {
    key: k,
    style: {
      position: 'absolute',
      left: 0,
  ...
  }, compact(tv)), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
  ...
  }, ref_ != null && /*#__PURE__*/React.createElement("line", {
    x1: "0",
    x2: "100",
    y1: yF(ref_),
    y2: yF(ref_),
    ...
    strokeLinecap: "round",
    vectorEffect: "non-scaling-stroke"
  }), act != null && /*#__PURE__*/React.createElement("line", {
    x1: xF(act) * 100,
    x2: xF(act) * 100,
    y1: "0",
    y2: "100",
  ...
  }, "piso"), act != null && /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
      left: `${xF(act) * 100}%`,
      top: `${yF(vals[act])}%`,
  ...
  }, fmt(vals[act]), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      opacity: .8,
      fontWeight: 600
    }
  }, data[act].label)), /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
      inset: 0,
      cursor: 'crosshair',
  ...
  }, (n <= 1 ? [0] : [...new Set([0, Math.floor((n - 1) / 2), n - 1])]).map(i => /*#__PURE__*/React.createElement("span", {
    key: i
  }, data[i].label))));
}
...
function MultiLineChart({
  ...
  if (n < 2) return /*#__PURE__*/React.createElement(Empty, null, "Lan\xE7a alguns torneios pra ver a curva.");
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    strokeDasharray: "2 2",
    vectorEffect: "non-scaling-stroke"
  }), series.map((s, si) => /*#__PURE__*/React.createElement("polyline", {
    key: si,
    points: s.values.map((v, i) => `${(xF(i) * 100).toFixed(2)},${yF(num(v)).toFixed(2)}`).join(' '),
    fill: "none",
    stroke: s.color,
    strokeWidth: "2",
    strokeLinejoin: "round",
    ...
    style: {
      ...
      left: `${xF(act) * 100}%`,
      top: '6%',
      transform: `translate(${xF(act) > 0.6 ? '-106%' : '6%'},0)`,
      background: C.ink,
      color: '#fff',
      padding: '6px 10px',
      borderRadius: 10,
  ...
  }, labels[act]), series.map((s, si) => /*#__PURE__*/React.createElement("div", {
    key: si,
    style: {
      display: 'flex',
      alignItems: 'center',
  ...
  }), s.name.split(' ')[0], ": ", fmt(num(s.values[act]))))), /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
      inset: 0,
      cursor: 'crosshair',
...
function Row({
  ...
  if (confirming) return /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8,
  ...
  }, "Remover este item?"), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setConfirming(false);
      onDelete();
    },
  ...
  }, left), right, onEdit && /*#__PURE__*/React.createElement("button", {
    onClick: onEdit,
    "aria-label": "Editar",
    style: {
      width: 34,
  ...
  }, /*#__PURE__*/React.createElement(IcoPencil, {
    s: 16
  ...
  }, /*#__PURE__*/React.createElement(IcoTrash, {
    s: 17
  })));
}
function ListView({
  ...
  }, subtitle)), onAdd && /*#__PURE__*/React.createElement("button", {
    onClick: onAdd,
    style: {
      display: 'flex',
      alignItems: 'center',
      ...
      cursor: 'pointer',
      flexShrink: 0,
      boxShadow: '0 6px 16px -8px rgba(91,75,138,.7)'
    }
  }, /*#__PURE__*/React.createElement(IcoPlus, {
    s: 18
  }), "Adicionar")), rows.length ? rows.map(renderRow) : /*#__PURE__*/React.createElement(Empty, null, empty));
}
...
function QuickEditModal({
  ...
  const init = kind === 'percent' ? String(num(currentValue) * 100).replace('.', ',') : kind === 'list' ? Array.isArray(currentValue) ? currentValue.join(', ') : String(currentValue || '') : kind === 'text' || kind === 'date' ? String(currentValue ?? '') : String(currentValue ?? 0).replace('.', ',');
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, hint), kind === 'select' ? /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 16
    }
  }, /*#__PURE__*/React.createElement(Select, {
    ...
    opts: opts || []
  })) : kind === 'date' ? /*#__PURE__*/React.createElement("input", {
    type: "date",
    style: {
      ...inputStyle,
      marginBottom: 16
    ...
    value: val,
    onChange: e => setVal(e.target.value)
  }) : kind === 'text' || kind === 'list' ? /*#__PURE__*/React.createElement("input", {
    style: {
      ...inputStyle,
      marginBottom: 16
    },
    ...
    autoFocus: true,
    placeholder: kind === 'list' ? 'Separe por vírgula' : ''
...
function AddModal({
  ...
  const save = () => {
    ...
    for (const fd of fields) {
      if ((fd.type === 'select' || fd.type === 'date') && !out[fd.k]) return;
    }
    editing ? onEdit({
      ...editing,
      ...out
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: labelStyle
  }, fd.label), fd.type === 'select' ? /*#__PURE__*/React.createElement(Select, {
    value: f[fd.k],
    onChange: e => set(fd.k, e.target.value),
    opts: fd.opts
  }) : fd.type === 'date' ? /*#__PURE__*/React.createElement("input", {
    type: "date",
    style: inputStyle,
    value: f[fd.k],
    onChange: e => set(fd.k, e.target.value)
  }) : fd.type === 'textarea' ? /*#__PURE__*/React.createElement("textarea", {
    rows: 2,
    style: {
      ...inputStyle,
      resize: 'vertical'
    ...
    value: f[fd.k],
    onChange: e => set(fd.k, e.target.value)
  }) : fd.type === 'money' ? /*#__PURE__*/React.createElement("input", {
    inputMode: "decimal",
    placeholder: "0,00",
    style: {
      ...inputStyle,
    ...
    value: f[fd.k],
    onChange: e => set(fd.k, e.target.value)
  }) : fd.type === 'int' ? /*#__PURE__*/React.createElement("input", {
    inputMode: "numeric",
    placeholder: "0",
    style: inputStyle,
    value: f[fd.k],
...
function PassInput({
  ...
  }, show ? /*#__PURE__*/React.createElement(IcoEyeOff, {
    s: 19
...
function RecoveryReset() {
  ...
  const save = async () => {
    ...
    if (error) {
      setBusy(false);
      return setErr('Não consegui trocar a senha. O link pode ter expirado — peça outro pelo "Esqueci minha senha".');
    }
  ...
  }, /*#__PURE__*/React.createElement(Brand, null)), ok ? /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: 'center'
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Senha alterada!"), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 13.5,
      color: C.inkSoft,
      lineHeight: 1.6,
  ...
  }, "Agora entra com a ", /*#__PURE__*/React.createElement("b", null, "senha nova"), ". Por seguran\xE7a, o acesso pelo link foi encerrado."), /*#__PURE__*/React.createElement("button", {
  ...
  }, "Ir para o login")) : /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 14
  ...
  }, "Crie sua nova senha"), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 13,
      color: C.inkSoft,
      textAlign: 'center',
  ...
  }, "Pra proteger a conta, voc\xEA precisa definir uma senha nova antes de entrar. M\xEDnimo 8 caracteres."), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Nova senha"), /*#__PURE__*/React.createElement(PassInput, {
    autoComplete: "new-password",
    value: p1,
    onChange: e => setP1(e.target.value)
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Confirmar nova senha"), /*#__PURE__*/React.createElement(PassInput, {
    autoComplete: "new-password",
    value: p2,
...
function ChangePassModal({
  ...
  }, "Senha trocada! \u2705"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13.5,
      color: C.inkSoft,
      lineHeight: 1.6,
  ...
  }, "Guarda ela num lugar seguro. Da pr\xF3xima vez, entra com a senha nova."), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      width: '100%',
      padding: '14px 0',
  ...
  }, "Escolhe uma senha s\xF3 sua (m\xEDnimo 8 caracteres). Toque no \uD83D\uDC41 pra conferir antes de salvar."), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Nova senha"), /*#__PURE__*/React.createElement(PassInput, {
    autoComplete: "off",
    value: p1,
...
function DeleteAccountModal({
  ...
  const go = async () => {
    ...
    const {
      error
    } = await sb.rpc('excluir_minha_conta');
    if (error) {
      setBusy(false);
      return setErr('Não consegui excluir agora. Tenta de novo em instantes.');
    }
    try {
      localStorage.removeItem('gb_signup');
      localStorage.removeItem('gb_tour');
    } catch (e) {}
    await sb.auth.signOut();
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Excluir minha conta"), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 13.5,
      color: C.inkSoft,
      lineHeight: 1.6,
  ...
  }, "Isso apaga ", /*#__PURE__*/React.createElement("b", null, "de forma definitiva"), " tudo que \xE9 seu: banca, torneios, estat\xEDsticas, m\xE3os importadas e o cadastro. ", /*#__PURE__*/React.createElement("b", null, "N\xE3o d\xE1 pra desfazer."), " Se voc\xEA paga um plano, cancele antes na Kiwify."), /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Digite ", /*#__PURE__*/React.createElement("b", null, alvo), " pra confirmar"), /*#__PURE__*/React.createElement("input", {
    style: {
      ...inputStyle,
      marginBottom: 12
    },
    ...
    onClick: go,
    disabled: busy || txt.trim().toUpperCase() !== alvo,
    style: {
      flex: 1,
      padding: '13px 0',
      borderRadius: 13,
      border: 'none',
      background: txt.trim().toUpperCase() === alvo ? C.red : C.bg,
      color: txt.trim().toUpperCase() === alvo ? '#fff' : '#C3BBA9',
      fontWeight: 700,
      fontSize: 14.5,
...
function Login() {
  ...
  const criar = async () => {
    ...
    try {
      const {
        data: livre
      } = await sb.rpc('nickname_disponivel', {
        nick: nome
      });
      if (livre === false) {
        setBusy(false);
        return setErr('Esse apelido já está em uso — escolhe outro (é único, como numa sala de poker).');
      }
    } catch (e) {}
    ...
    if (!data || !data.session) {
      setBusy(false);
      setMode('login');
      setOk('Conta criada! Confirma no teu e-mail e depois entra aqui — teu apelido fica guardado.');
      return;
    }
    ...
    if (pe && pe.code === '23505') {
      setBusy(false);
      return setErr('Esse apelido acabou de ser registrado por outra pessoa — escolhe outro.');
    }
    if (!pe) {
      try {
        localStorage.removeItem('gb_signup');
  ...
  const entrar = async () => {
    ...
    try {
      ({
        data,
        error
      } = await sb.functions.invoke('auth-alias', {
        body: {
          action: 'login',
          identifier: id,
          password: pass
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement(Brand, null)), mode === 'login' ? /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 14
    ...
    style: labelStyle
  }, "Senha"), /*#__PURE__*/React.createElement(PassInput, {
  ...
  }, busy ? 'Entrando...' : 'Entrar'), /*#__PURE__*/React.createElement("button", {
    onClick: esqueci,
    disabled: busy,
    style: {
      border: 'none',
  ...
  }, "Criar conta gr\xE1tis")) : /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 14
  ...
  }, "Sua conta individual: banca, torneios e estat\xEDsticas \u2014 s\xF3 seus, ningu\xE9m mais v\xEA."), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Nome ou apelido"), /*#__PURE__*/React.createElement("input", {
    style: inputStyle,
    value: nNome,
    autoCapitalize: "none",
    placeholder: "Ex: rafa_grinder",
    ...
    style: inputStyle,
    inputMode: "tel",
    placeholder: "DDD + n\xFAmero",
    value: nZap,
    onChange: e => setNZap(e.target.value)
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Senha"), /*#__PURE__*/React.createElement(PassInput, {
    autoComplete: "new-password",
    ...
    style: labelStyle
  }, "Confirmar senha"), /*#__PURE__*/React.createElement(PassInput, {
    autoComplete: "new-password",
    value: nP2,
  ...
  }, "Ao criar a conta voc\xEA concorda com os ", /*#__PURE__*/React.createElement("a", {
    href: "termos.html",
    target: "_blank",
    style: {
      color: P
    }
  }, "Termos de Uso"), " e a ", /*#__PURE__*/React.createElement("a", {
    href: "privacidade.html",
    target: "_blank",
    style: {
      color: P
    }
  }, "Pol\xEDtica de Privacidade"), "."), /*#__PURE__*/React.createElement("button", {
    onClick: criar,
    disabled: busy,
    style: {
      padding: '15px 0',
  ...
  }, busy ? 'Criando…' : 'Criar minha conta'), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setMode('login');
      setErr('');
    },
...
function SetupSolo({
    ...
    try {
      ...
      if (!jaTem || !jaTem.length) {
        ...
        const {
          ...
          modalidades_permitidas: ['MTT', 'Spin', 'Cash', 'Sit & Go']
        });
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Como te chamamos?"), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Nome ou apelido"), /*#__PURE__*/React.createElement("input", {
    style: inputStyle,
    value: nome,
  ...
  }, "Sua banca hoje"), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "Banca inicial (US$) \u2014 pode ser 0"), /*#__PURE__*/React.createElement("input", {
    style: inputStyle,
    inputMode: "decimal",
    value: banca,
    placeholder: "Ex: 150",
    ...
    style: labelStyle
  }, "Onde voc\xEA joga"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap',
  ...
  }, ['GG Poker', 'PokerStars', 'Outros'].map(s => /*#__PURE__*/React.createElement("button", {
    key: s,
    onClick: () => toggleSite(s),
    style: {
      padding: '8px 13px',
      borderRadius: 99,
      border: `1.5px solid ${sites.includes(s) ? P : C.border}`,
      background: sites.includes(s) ? C.plumSoft : 'transparent',
      color: sites.includes(s) ? P : C.inkSoft,
      fontWeight: 700,
      fontSize: 13,
      cursor: 'pointer'
    }
  ...
  }, "Tudo pronto, ", nome.trim().split(' ')[0], "!"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13.5,
      color: C.inkSoft,
      lineHeight: 1.7
    }
  }, "\u2713 Lan\xE7a seus torneios na aba ", /*#__PURE__*/React.createElement("b", null, "Torneios"), /*#__PURE__*/React.createElement("br", null), "\u2713 Acompanha banca e semana no ", /*#__PURE__*/React.createElement("b", null, "Painel"), /*#__PURE__*/React.createElement("br", null), "\u2713 Importa suas m\xE3os na aba ", /*#__PURE__*/React.createElement("b", null, "Stats"), " pra ver suas estat\xEDsticas e leituras"), err && /*#__PURE__*/React.createElement("div", {
    style: {
      color: C.red,
      fontSize: 13.5,
      fontWeight: 600
...
function Onboarding({
  ...
  const save = async () => {
    ...
    if (!(profile && profile.nickname && profile.nickname.toLowerCase() === nickname.toLowerCase())) {
      try {
        ...
        if (livre === false) {
          setBusy(false);
          return setErr('Esse nickname já está em uso — escolhe outro (é único, como numa sala de poker).');
        }
      } catch (e) {}
    }
    const {
    ...
    if (e1) {
      setBusy(false);
      return setErr('Não consegui trocar a senha. Tenta de novo.');
    }
    const row = {
    ...
    const {
      data,
      error: e2
    } = await sb.from('player_profiles').upsert(row).select().single();
    setBusy(false);
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement(Brand, null)), /*#__PURE__*/React.createElement("h3", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 21,
      fontWeight: 600,
  ...
  }, "Primeiro acesso"), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 13.5,
      color: C.inkSoft,
      textAlign: 'center',
    ...
    value: nick,
    autoCapitalize: "none",
    placeholder: "Ex: gab_gg",
    onChange: e => setNick(e.target.value)
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: labelStyle
  }, "CPF (opcional, s\xF3 n\xFAmeros)"), /*#__PURE__*/React.createElement("input", {
    style: inputStyle,
    inputMode: "numeric",
    value: cpf,
  ...
  }, "Toque no \uD83D\uDC41 pra conferir a senha antes de salvar. Anote ou salve num lugar seguro."), err && /*#__PURE__*/React.createElement("div", {
    style: {
      color: C.red,
      fontSize: 13.5,
      fontWeight: 600
...
function NotConfigured() {
  ...
  }, "Falta conectar o banco de dados"), /*#__PURE__*/React.createElement("p", {
    style: {
      color: C.inkSoft,
      fontSize: 14.5,
      lineHeight: 1.6,
  ...
  }, "Abra o ", /*#__PURE__*/React.createElement("b", null, "index.html"), " e cole a ", /*#__PURE__*/React.createElement("b", null, "URL"), " e a ", /*#__PURE__*/React.createElement("b", null, "chave anon"), " do projeto Supabase da pool no topo do arquivo. Depois recarregue.")));
}
...
function TourRow({
  ...
  return /*#__PURE__*/React.createElement(Row, {
    ...
    }, /*#__PURE__*/React.createElement(IcoTrophy, {
      s: 18
    })), /*#__PURE__*/React.createElement("div", {
      style: {
        minWidth: 0
    ...
    }, t.tournament_name || 'Torneio', " ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: col,
        fontWeight: 700
      }
    }, "\xB7 ", t.player)), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12,
        color: C.inkSoft,
        display: 'flex',
    ...
    right: /*#__PURE__*/React.createElement("span", {
      style: {
        fontWeight: 800,
        fontSize: 15,
        color: lp >= 0 ? C.greenMid : C.red,
        flexShrink: 0
      }
    }, lp >= 0 ? '+' : '−', fmt(Math.abs(lp)))
  });
}
...
function Seg({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      padding: '6px 11px',
      borderRadius: 99,
      border: `1.5px solid ${value === o ? P : C.border}`,
      background: value === o ? C.plumSoft : C.surface,
      color: value === o ? P : C.inkSoft,
      fontWeight: 700,
      fontSize: 12.5,
      cursor: 'pointer'
    }
...
function StatTile({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      fontSize: 18,
      fontWeight: 600,
      color: has ? tone : C.inkSoft
    }
  }, has ? pctFmt(pct) : '—'), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10.5,
      color: C.inkSoft
    }
...
function DayCard({
  ...
  const per = players.map((p, i) => {
    const ts = dayTours.filter(t => t.player === p);
    return {
      p,
      i,
      n: ts.length,
  ...
  return /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 0,
      overflow: 'hidden',
      border: today ? `1.5px solid ${P}` : `1px solid ${C.border}`
    }
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setOpen(o => !o),
    style: {
      ...
      gap: 10,
      padding: '15px 18px',
      background: today ? C.plumSoft : 'transparent',
      border: 'none',
      cursor: 'pointer',
      textAlign: 'left'
    }
  ...
  }, today ? 'Hoje · ' : '', dLabel(date)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12.5,
      color: C.inkSoft,
      marginTop: 2
    }
  }, dayTours.length, " torneio", dayTours.length !== 1 ? 's' : '', per.length ? ' · ' + per.map(x => x.p.split(' ')[0]).join(' + ') : '')), /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontWeight: 700,
      fontSize: 17,
      color: res >= 0 ? C.greenMid : C.red
    }
  }, res >= 0 ? '+' : '−', fmt(Math.abs(res))), /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.inkSoft,
      fontSize: 20,
  ...
  }, per.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap',
  ...
  }, per.map(x => /*#__PURE__*/React.createElement("span", {
    key: x.p,
    style: {
      display: 'flex',
      alignItems: 'center',
    ...
    style: {
      ...
      height: 8,
      borderRadius: 99,
      background: PLAYER_COLORS[x.i]
    }
  }), x.p.split(' ')[0], ": ", x.res >= 0 ? '+' : '−', fmt(Math.abs(x.res)), " ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.inkSoft,
      fontWeight: 600
    }
  }, "(", x.n, ")")))), sortByCreated(dayTours).map(t => /*#__PURE__*/React.createElement(TourRow, {
    key: t.id,
    t: t,
    config: config,
    players: players,
    ...
    s: 16
  }), "Adicionar torneio ", today ? 'de hoje' : `em ${dLabel(date)}`)));
}
function DiaryCard({
  entry,
  dayTours,
  ...
  const st = gradeStatus(entry, abiMax),
    r = resultadoDia(entry),
    i = players.indexOf(entry.player);
  ...
  return /*#__PURE__*/React.createElement(Card, {
  ...
  }, entry.player, " ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.inkSoft,
      fontWeight: 600
    }
  }, "\xB7 ", dLabel(entry.entry_date))), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: C.inkSoft,
      display: 'flex',
  ...
  }, num(entry.qtd_torneios), " torneios", ent !== num(entry.qtd_torneios) ? ` (${ent} entradas)` : '', " \xB7 ABI ", fmt(abiMedioDia(entry)), " ", st === 'FORA DA GRADE' && /*#__PURE__*/React.createElement(Badge, {
    text: st
  }))), /*#__PURE__*/React.createElement("span", {
    style: {
      fontWeight: 800,
      fontSize: 16,
      color: r >= 0 ? C.greenMid : C.red
    }
  }, r >= 0 ? '+' : '−', fmt(Math.abs(r))), /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.inkSoft,
      fontSize: 20,
      transform: open ? 'rotate(90deg)' : 'none',
    ...
    label: "ABI m\xE9dio",
    value: fmt(abiMedioDia(entry))
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: `ABI máx (${entry.player.split(' ')[0]})`,
    value: fmt(abiMax)
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "Investido",
    value: fmt(entry.total_buyins)
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "Premia\xE7\xE3o",
    value: fmt(premios),
    tone: C.greenMid
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "Cashes (ITM)",
    value: `${cashes}/${num(entry.qtd_torneios)}`
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "ROI do dia",
    value: pctFmt(roi),
    tone: r >= 0 ? C.greenMid : C.red
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: C.inkSoft,
  ...
  }, "Corrija editando os torneios abaixo:"), sortByCreated(dayTours).map(t => /*#__PURE__*/React.createElement(TourRow, {
    key: t.id,
    t: t,
    config: config,
    players: players,
    ...
    onClick: () => onAdd(entry.entry_date, entry.player),
    style: {
      marginTop: 12,
      display: 'flex',
      alignItems: 'center',
    ...
    s: 16
  }), "Adicionar torneio de ", entry.player.split(' ')[0])));
}
...
function WeekRow({
  ...
  }, w.resultado >= 0 ? '+' : '−', fmt(Math.abs(w.resultado)))), !solo && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
  ...
  }, "Make-up: ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: C.ink
    }
  }, fmt(w.makeAnterior), " \u2192 ", fmt(w.makeFinal))), /*#__PURE__*/React.createElement("div", {
    style: {
      color: C.inkSoft
    }
  }, "Lucro divis\xEDvel: ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: C.ink
    }
  }, fmt(w.lucroDiv))), /*#__PURE__*/React.createElement("div", {
    style: {
      color: C.inkSoft
    }
  }, "Parte do jogador: ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: C.ink
    }
  }, fmt(w.parteJog))), /*#__PURE__*/React.createElement("div", {
    style: {
      color: C.inkSoft
    }
  }, "Saque autorizado: ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: P
    }
  }, fmt(w.saqueAut)))), wkTorn > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 8
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "ABI m\xE9dio da semana ", fmt(wkAbi), " / m\xE1x ", fmt(pAbi)), /*#__PURE__*/React.createElement(Bar2, {
...
function SemanalGeral({
  ...
  }, "saque autorizado ", fmt(g.saqueAut), " \xB7 pool ficou com ", fmt(g.partePool))), /*#__PURE__*/React.createElement("span", {
    style: {
      fontWeight: 800,
      fontSize: 15.5,
      color: g.resultado >= 0 ? C.greenMid : C.red,
      flexShrink: 0
    }
  }, g.resultado >= 0 ? '+' : '−', fmt(Math.abs(g.resultado))));
  return /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 20
    }
  ...
  }, "Resultado por semana (geral da pool)"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12.5,
      color: C.inkSoft,
      marginBottom: 8
    }
  }, "Os dois jogadores somados, semana a semana."), atuais.length ? atuais.map(Row) : /*#__PURE__*/React.createElement(Empty, null, "Sem semanas neste m\xEAs ainda."), antigas.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(HistToggle, {
    n: antigas.length,
    open: hist,
    onClick: () => setHist(h => !h)
...
function SemanalPlayer({
  ...
  return /*#__PURE__*/React.createElement(Card, {
    ...
    style: {
      ...
      height: 12,
      borderRadius: 99,
      background: PLAYER_COLORS[[config.player1_name, config.player2_name].indexOf(player)] || C.inkSoft
    }
  }), /*#__PURE__*/React.createElement("h3", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
  ...
  }, player), !solo && /*#__PURE__*/React.createElement("span", {
    style: {
      marginLeft: 'auto',
      fontSize: 13,
      color: C.inkSoft,
  ...
  }, "make-up atual ", fmt(makeUpAtual))), atuais.length ? atuais.map(w => /*#__PURE__*/React.createElement(WeekRow, {
    key: w.week,
    w: w,
    player: player,
    config: config,
    solo: solo
  })) : /*#__PURE__*/React.createElement(Empty, null, "Sem semanas neste m\xEAs ainda."), antigas.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(HistToggle, {
    n: antigas.length,
    open: hist,
    onClick: () => setHist(h => !h)
  }), hist && /*#__PURE__*/React.createElement("div", {
  ...
  }, antigas.map(w => /*#__PURE__*/React.createElement(WeekRow, {
    key: w.week,
    w: w,
    player: player,
    config: config,
...
function Dashboard({
  ...
  const [stF, setStF] = useState({
    ...
    from: '',
    to: ''
  }); // filtros da tela Stats (preset ou intervalo de datas)
  ...
  useEffect(() => {
    if (isStandalone()) {
      ...
      return;
    } // rodando como app: some de vez
    let v = '';
    try {
      v = localStorage.getItem('gb_install') || '';
    } catch (e) {}
    if (v === '1') return; // dispensado / já instalado
    const podeMostrar = () => {
      let x = '';
      try {
        x = localStorage.getItem('gb_install') || '';
      } catch (e) {}
      return x !== '1' && (parseInt(x, 10) || 0) < 2;
    };
    if ((window.__deferredInstall || isIOS()) && podeMostrar()) {
      setInstallCard(true);
      try {
        localStorage.setItem('gb_install', String((parseInt(v, 10) || 0) + 1));
      } catch (e) {} // conta esta exibição (para de mostrar após 2)
    }
    const onBip = () => {
      if (!isStandalone() && podeMostrar()) setInstallCard(true);
    };
    const onInstalled = () => {
  ...
  const loadAll = async () => {
    try {
      ...
      try {
        await sb.rpc('aplicar_ativacao');
      } catch (e) {}
      // carga UNIFORME: o RLS por workspace entrega a cada conta só o que é dela
      const [wsr, cfg, t, l, pw, w, hhr, cc] = await Promise.all([sb.from('workspaces').select('*'), sb.from('pool_config').select('*').order('created_at', {
        ascending: true
      }), sb.from('tournaments').select('*').order('entry_date', {
        ascending: false
      }), sb.from('bankroll_ledger').select('*').order('entry_date'), sb.from('player_wallets').select('*'), sb.from('withdrawals').select('*').order('week_ending_date', {
        ascending: false
      }), sb.from('hh_tournament_stats').select('*').order('entry_date', {
        ascending: false
      }), sb.from('config_changes').select('*').order('created_at', {
        ascending: false
      })]);
      const myWs = wsr.data && wsr.data[0] || null;
      setWs(myWs);
      ...
      if (!c && !cfg.error) {
        ...
        c = r.data || null;
        if (!c) {
          const r2 = await sb.from('pool_config').select('*').order('created_at', {
            ascending: true
          });
          c = r2.data && r2.data[0] || null;
        }
      }
      if (!c) {
        console.error('pool_config indisponível, tentando de novo…', cfg.error);
        if (!configRef.current) setTimeout(loadAll, 2500);
        return;
      }
      setNeedsSetup(false);
      setConfig(c);
  ...
  useEffect(() => {
    ...
    const onTour = p => {
      ...
      if (p.eventType === 'INSERT' && p.new && p.new.player && !localIds.current.has(p.new.id) && configRef.current && !soloRef.current) {
        ...
        if (num(p.new.buyin) > mx) pushToast({
          tone: C.red,
          title: `${p.new.player} jogou fora da grade`,
          text: `${p.new.tournament_name || 'Torneio'} · buy-in ${fmt(p.new.buyin)} (máx ${fmt(mx)}). Fique atento e verifique com ${p.new.player.split(' ')[0]}.`
        });
      }
    };
    const ch = sb.channel('pool-sync').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'tournaments'
    }, onTour).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'bankroll_ledger'
    }, apply(setLedger)).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'player_wallets'
    }, apply(setWallets)).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'hh_tournament_stats'
    }, apply(setHh)).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'withdrawals'
    }, apply(setWds)).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'pool_config'
    }, p => {
      if (p.new && p.new.id) setConfig(prev => prev && prev.id === p.new.id ? p.new : prev);
    })
    ...
    .on('postgres_changes', {
      ...
      apply(setChgs)(p);
      if (p.eventType === 'INSERT' && p.new && !localIds.current.has(p.new.id)) pushToast({
        tone: C.gold,
        title: `${p.new.changed_by_name || 'Alguém'} alterou os Ajustes`,
        text: p.new.resumo
      });
    }).subscribe();
    return () => {
  ...
  const add = async (table, data, setter, list) => {
    const {
      data: row,
      error
    } = await sb.from(table).insert(data).select().single();
    if (error) {
  ...
  const update = async (table, newData, setter, list) => {
    ...
    const {
      data: updated,
      error
    } = await sb.from(table).update(payload).eq('id', id).select().single();
    if (error) {
      alert('Não consegui atualizar. Tente de novo.');
      console.error(error);
      return;
    }
    setter(list.map(x => x.id === id ? updated : x));
    setEditing(null);
  };
  const saveConfig = async patch => {
    ...
    const {
      error
    } = await sb.from('pool_config').update(patch).eq('id', config.id);
    if (error) {
      alert('Não consegui salvar a configuração — nada foi alterado.');
      console.error(error);
      setConfig(prev);
      return false;
    }
  ...
  const askSaveConfig = (row, nv) => {
    ...
    setCfgConfirm({
      row,
      nv,
      oldShow: showCfgVal(row, old),
      newShow: showCfgVal(row, nv)
    });
  };
  const confirmSaveConfig = async () => {
    ...
    const {
      data,
      error
    } = await sb.from('config_changes').insert({
      field: row.key,
    ...
    if (error) {
      console.error('config_changes', error);
      return;
    }
    if (data && data[0]) {
      localIds.current.add(data[0].id);
      setChgs(l => l.some(x => x.id === data[0].id) ? l : [data[0], ...l]);
    }
  };
  ...
  const saveWallet = async (player, wallet, saldo_inicial) => {
    ...
    const {
      data,
      error
    } = await sb.from('player_wallets').upsert({
      player,
      wallet,
      saldo_inicial
    }, {
      onConflict: 'workspace_id,player,wallet'
    }).select().single();
    if (error) {
      alert('Não consegui salvar o saldo.');
      console.error(error);
      return;
    }
    setWallets(w => {
      const rest = w.filter(x => !(x.player === player && x.wallet === wallet));
      return [...rest, data];
    });
  };
  ...
  const addTransfer = async out => {
    ...
    if (deP === paraP && out.de_wallet === out.para_wallet) {
      alert('Origem e destino são a mesma carteira.');
      return;
    }
    ...
    const rows = [{
      ...
      player: deP,
      wallet: out.de_wallet,
      entrada: 0,
      saida: val,
      observacao: `Transferência → ${out.para_wallet} (${nome(paraP)})${obs}`
    }, {
      entry_date: out.entry_date,
      player: paraP,
      wallet: out.para_wallet,
      entrada: val,
      saida: 0,
      observacao: `Transferência ← ${out.de_wallet} (${nome(deP)})${obs}`
    }];
    const {
      data,
      error
    } = await sb.from('bankroll_ledger').insert(rows).select();
    if (error) {
      alert('Não consegui registrar a transferência.');
      console.error(error);
      return;
    }
  ...
  const importHH = async (files, player) => {
    ...
    const addZip = (name, u, depth) => {
      if (!window.fflate) {
        issues.push(name + ': leitor de ZIP não carregou (recarregue a página e tente de novo)');
        return;
      }
      let entries;
      try {
        ...
        issues.push(name + ': zip inválido ou protegido (' + e.message + ')');
        return;
      }
      let usou = false,
        planilhas = 0;
      for (const [n, b] of Object.entries(entries)) {
        if (diag.zips.length < 60) diag.zips.push({
          zip: name,
          entry: n,
        ...
        if (/\.zip$/i.test(n) || isZip(b)) {
          if (depth < 3) {
            addZip(name + ' → ' + n, b, depth + 1);
            usou = true;
          }
          continue;
        }
        if (/\.(csv|xlsx?|pdf)$/i.test(n)) {
          planilhas++;
          continue;
        }
        const tx = decodeSmart(b);
        if (/\.txt$/i.test(n) || HH_START.test(tx)) {
          texts.push({
            name: n,
            text: tx
          });
      ...
      if (!usou) issues.push(name + (planilhas ? ': isso parece o export de RESULTADOS (planilha/PDF) do PokerCraft — baixe o HAND HISTORY (as mãos), não o resumo' : ': nenhum arquivo de mãos dentro do zip'));
    };
    for (const f of files) {
      try {
        const u = new Uint8Array(await f.arrayBuffer());
        diag.files.push({
          name: f.name,
          kb: Math.round((f.size || u.length) / 1024),
          magic: Array.from(u.slice(0, 4)).map(x => x.toString(16).padStart(2, '0')).join('')
        });
        if (/\.zip$/i.test(f.name) || isZip(u)) addZip(f.name, u, 0);else texts.push({
          name: f.name,
          text: decodeSmart(u)
        });
      } catch (e) {
    ...
    for (let i = 0; i < texts.length; i++) {
      setImp(s => ({
        ...s,
        prog: `Analisando ${i + 1}/${texts.length}: ${texts[i].name}`
      }));
      ...
      for (let j = 0; j < parts.length; j += 400) {
        ...
        allHands = allHands.concat(r.hands);
        ignored += r.ignored;
        achou += r.hands.length + r.ignored;
        maosArq += r.hands.length;
        Object.keys(r.reasons || {}).forEach(k => {
        ...
        if (parts.length > 400) {
          setImp(s => ({
            ...s,
            prog: `Analisando ${i + 1}/${texts.length}: ${texts[i].name} (${Math.min(j + 400, parts.length)} de ${parts.length} mãos)`
          }));
      ...
      if (!achou) {
        if (/^\s*Tournament #\S+,/.test(String(texts[i].text || ''))) resumos++;else issues.push(texts[i].name + ': nenhuma mão reconhecida — é um hand history do PokerStars (.txt) ou do PokerCraft (GG)?');
      }
      if (diag.texts.length < 60) diag.texts.push({
        name: texts[i].name,
        head: String(texts[i].text || '').slice(0, 200).split(/\r?\n/)[0].slice(0, 120),
        maos: maosArq,
        ign: achou - maosArq
      });
    }
    ...
    for (const k of Object.keys(grupos)) {
      ...
      const upgrade = existente && Array.isArray(existente.hand_ids) && existente.hand_ids.length && existente.pos_json && Object.values(existente.pos_json).some(x => x && x.hn == null) && existente.hand_ids.every(id => batchIds.has(id));
      ...
      for (const h of g) {
        ...
        if (jaVistas.has(id) || noLote.has(id)) {
          repetidas++;
          continue;
        }
        noLote.add(id);
        frescas.push(h.hid ? h : {
          ...h,
          hid: id
        });
      }
      if (upgrade) {
        reproc += frescas.length;
      } else {
        novas += frescas.length;
      }
    ...
    for (let i = 0; i < paraSalvar.length; i += 100) {
      ...
      setImp(s => ({
        ...s,
        prog: `Salvando torneios ${i + 1}–${Math.min(i + 100, paraSalvar.length)} de ${paraSalvar.length}…`
      }));
      const {
        data,
        error
      } = await sb.from('hh_tournament_stats').upsert(chunk, {
        onConflict: 'workspace_id,player,site,site_tournament_id'
      }).select();
      if (error) {
        issues.push(`lote de torneios ${i + 1}+: não salvou (${error.message})`);
        continue;
      }
      const recs = data && data.length === chunk.length ? data : chunk;
      saved += chunk.length;
      setHh(list => {
        const novasKeys = new Set(recs.map(keyOf));
        return [...recs, ...list.filter(x => !novasKeys.has(keyOf(x)))];
      });
      await new Promise(r => setTimeout(r, 0));
    }
    setImp({
      ...
      res: {
        ...
        repetidas,
        reproc,
        tours: Object.keys(grupos).length,
        saved,
        ignored,
        reasons,
    ...
    try {
      await sb.from('hh_import_log').insert({
        player,
        saved,
        novas,
        repetidas,
  ...
  if (needsSetup && !config) return /*#__PURE__*/React.createElement(SetupSolo, {
    profile: profile,
    ws: ws,
    onDone: () => {
      setNeedsSetup(false);
      if (!localStorage.getItem('gb_tour')) setAskTour(true);
      loadAll();
    }
  });
  if (loading || !config) return /*#__PURE__*/React.createElement("div", {
    style: {
      minHeight: '100vh',
      display: 'grid',
      placeItems: 'center'
  ...
  const abiMax = num(config.abi_max),
    piso = num(config.piso_minimo);
  ...
  const walletInicial = (player, wallet) => {
    const w = wallets.find(x => x.player === player && x.wallet === wallet);
    return w ? num(w.saldo_inicial) : 0;
  };
  ...
  const saldoWallet = (player, wallet) => walletInicial(player, wallet) + tours.filter(t => t.player === player && walletBucket(t.site) === wallet).reduce((s, t) => s + lucroTorneio(t), 0) + ledger.filter(l => l.player === player && walletBucket(l.wallet) === wallet).reduce((s, l) => s + num(l.entrada) - num(l.saida), 0) - wds.filter(w => w.player === player && walletBucket(w.wallet) === wallet).reduce((s, w) => s + num(w.valor_sacado), 0);
  const saldoJogador = player => WALLETS.reduce((s, wl) => s + saldoWallet(player, wl), 0);
  const walletsDoJogador = player => WALLETS.map(wl => ({
    wallet: wl,
    saldo: saldoWallet(player, wl),
    inicial: walletInicial(player, wl),
    temMov: walletInicial(player, wl) !== 0 || tours.some(t => t.player === player && walletBucket(t.site) === wl) || ledger.some(l => l.player === player && walletBucket(l.wallet) === wl) || wds.some(w => w.player === player && walletBucket(w.wallet) === wl)
  })).filter(x => x.temMov);
  const curMakeUp = {},
    curWeek = {};
  players.forEach(p => {
    const arr = weeksByPlayer[p] || [];
    curMakeUp[p] = arr.length ? arr[arr.length - 1].makeFinal : makeInit[p] || 0;
    curWeek[p] = arr.find(w => w.week === CURWK) || null;
  });
  const valorSacadoFor = (week, player) => wds.filter(w => w.week_ending_date === week && w.player === player).reduce((s, w) => s + num(w.valor_sacado), 0);
  const makeUpAt = (p, wk) => {
    let mk = makeInit[p] || 0;
    for (const w of weeksByPlayer[p] || []) {
      if (w.week <= wk) mk = w.makeFinal;else break;
    }
    return mk;
  };
  ...
  let saqueAutTotal = 0;
  {
    let bancaLivre = bancaAtual;
    players.forEach(p => {
      const w = curWeek[p];
      if (!w || w.saqueAut === 0 || w.makeFinal > 0) return;
      if (bancaLivre - w.saqueAut < piso) return;
      bancaLivre -= w.saqueAut;
      saqueAutTotal += w.saqueAut;
    });
  }
  ...
  const resChart = chartWeeks.map(wk => ({
    label: dLabel(wk),
    vals: players.map(p => {
      const w = (weeksByPlayer[p] || []).find(x => x.week === wk);
      return w ? w.resultado : 0;
    })
  }));
  const makeChart = chartWeeks.map(wk => ({
    label: dLabel(wk),
    vals: players.map(p => makeUpAt(p, wk))
  }));
  const volWeeks = [...new Set(daily.map(e => weekEnding(e.entry_date)))].sort().slice(-8);
  const volChart = volWeeks.map(wk => ({
    label: dLabel(wk),
    vals: [daily.filter(e => weekEnding(e.entry_date) === wk).reduce((s, e) => s + num(e.qtd_torneios), 0)]
  }));
  const bankWeeks = [...new Set([...daily.map(e => weekEnding(e.entry_date)), ...ledger.map(l => weekEnding(l.entry_date)), ...wds.map(w => weekEnding(w.week_ending_date))])].sort();
  let runBanca = num(config.banca_inicial);
  const bancaChart = [{
    label: 'Início',
    value: runBanca
  }].concat(bankWeeks.map(wk => {
  ...
  const lucroSeries = players.map((p, i) => {
    let run = 0;
    const vals = [0, ...lucroDias.map(d => {
      run += daily.filter(e => e.player === p && e.entry_date === d).reduce((s, e) => s + resultadoDia(e), 0);
      return run;
    })];
    return {
      name: p,
      color: PLAYER_COLORS[i],
      values: vals.slice(-13)
    };
  });

  /* atalhos de torneio (lançar/editar/excluir a partir do Diário ou dos cards de dia) */
  const addTourOn = (date, player) => setModal({
    type: 'tour',
    initial: {
  ...
  const FIELDS = {
    ...
    bank: [{
      ...
      k: 'player',
      label: 'Jogador (ou pool)',
      type: 'select',
      opts: ['Pool / Reserva', ...players]
    }, {
      k: 'wallet',
      label: 'Carteira',
      type: 'select',
    ...
    wd: [{
      k: 'week_ending_date',
      label: 'Semana (domingo de fechamento)',
      type: 'date',
      def: CURWK,
      full: true
    }, {
      ...
      k: 'wallet',
      label: 'Saiu de qual carteira',
      type: 'select',
      opts: ['Não informar', ...WALLETS]
    }, {
      k: 'valor_sacado',
      label: 'Valor sacado (US$)',
      type: 'money',
      def: '0'
    }, {
      k: 'observacoes',
    ...
    transfer: [{
      ...
      label: 'Vai pra quem',
      type: 'select',
      opts: [...players, 'Pool / Reserva']
    }, {
      k: 'para_wallet',
      label: 'Vai pra qual carteira',
      type: 'select',
      opts: WALLETS,
      def: 'PokerStars'
    }, {
  ...
  const MODAL_TITLE = {
    tour: 'torneio',
    bank: 'movimento na banca',
    wd: 'saque pago'
  };
  const MODAL_TABLE = {
    tour: 'tournaments',
    bank: 'bankroll_ledger',
    wd: 'withdrawals'
  };
  const MODAL_STATE = {
    tour: [tours, setTours],
    bank: [ledger, setLedger],
    wd: [wds, setWds]
  };
  ...
  const breakdownBy = (key, list) => {
    ...
    list.forEach(t => {
      ...
      const g = m[k];
      g.n += 1;
      g.inv += totalInvestido(t);
      g.res += lucroTorneio(t);
      g.premios += num(t.prize);
    });
    return Object.values(m).map(g => ({
      ...g,
      roi: g.inv > 0 ? g.res / g.inv * 100 : 0
    })).sort((a, b) => b.res - a.res);
  };
  const bySite = breakdownBy('site', monthToursWho),
    byModality = breakdownBy('modality', monthToursWho);
  ...
  const diPeriodo = (() => {
    // resolve o período ativo -> {from,to} (ou null)
    ...
    if (diF.from || diF.to) return {
      from: diF.from || '0000-01-01',
      to: diF.to || '9999-12-31',
      label: `${diF.from ? dLabel(diF.from) : 'início'} a ${diF.to ? dLabel(diF.to) : 'hoje'}`
    };
    return null;
  })();
  const diaryTours = tours.filter(t => (diF.player === 'Todos' || t.player === diF.player) && (diF.site === 'Todos' || t.site === diF.site) && (diF.mod === 'Todos' || t.modality === diF.mod) && (!diPeriodo || t.entry_date >= diPeriodo.from && t.entry_date <= diPeriodo.to));
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }), nav.map(n => /*#__PURE__*/React.createElement(NavBtn, {
    key: n.id,
    Icon: n.Icon,
    label: n.label,
    active: view === n.id,
  ...
  }, "Banca atual"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 24,
      fontWeight: 600,
  ...
  }, fmt(bancaAtual))), /*#__PURE__*/React.createElement("button", {
    onClick: sair,
    style: {
      padding: '10px',
      borderRadius: 12,
    ...
    s: 16
  }), "Sair")), view === 'painel' && /*#__PURE__*/React.createElement("div", {
    className: "ftfade",
    style: {
      display: 'flex',
      flexDirection: 'column',
  ...
  }, /*#__PURE__*/React.createElement(IcoStack, {
    s: 17
  }), solo ? 'Minha banca' : 'Banca central da pool'), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 42,
      fontWeight: 600,
  ...
  }, fmt(bancaAtual)), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: '6px 16px',
      flexWrap: 'wrap',
  ...
  }, /*#__PURE__*/React.createElement("span", null, "Inicial ", fmt(config.banca_inicial)), /*#__PURE__*/React.createElement("span", null, "Jogos ", resultadosTotal >= 0 ? '+' : '−', fmt(Math.abs(resultadosTotal))), movimentosTotal !== 0 && /*#__PURE__*/React.createElement("span", null, "Aportes ", movimentosTotal >= 0 ? '+' : '−', fmt(Math.abs(movimentosTotal))), sacadoTotal > 0 && /*#__PURE__*/React.createElement("span", null, "Saques \u2212", fmt(sacadoTotal))), /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 12
    }
  }, /*#__PURE__*/React.createElement(Bar2, {
    pct: piso > 0 ? bancaAtual / piso * 100 : 100,
    color: "#fff",
    ...
    Icon: resSemanaAtual >= 0 ? IcoUp : IcoDown,
    tone: resSemanaAtual >= 0 ? C.greenMid : C.red,
    ...
    bg: makeUpTotal > 0 ? C.goldSoft : C.greenSoft,
    label: "Make-up em aberto",
    value: fmt(makeUpTotal),
    sub: players.map(p => `${p.split(' ')[0]}: ${fmt(curMakeUp[p])}`).join(' · ')
  }), !solo && /*#__PURE__*/React.createElement(Stat, {
    Icon: IcoCashOut,
    tone: P,
    bg: C.plumSoft,
    label: "Saque autorizado",
    value: fmt(saqueAutTotal),
    sub: "semana atual, respeitando o piso"
  }), /*#__PURE__*/React.createElement(Stat, {
    Icon: IcoChip,
    tone: C.green,
    bg: C.greenSoft,
  ...
  }), "A receber \xB7 ", p), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 22,
      fontWeight: 600,
  ...
  }, fmt(aReceber[p])))), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: 14,
      borderRadius: 14,
      background: C.greenSoft
  ...
  }, "Fica na banca (pool)"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 22,
      fontWeight: 600,
  ...
  }, "Lucro acumulado por jogador"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12.5,
      color: C.inkSoft,
      marginBottom: 12
  ...
  }, "Evolu\xE7\xE3o da banca"), /*#__PURE__*/React.createElement(LineChart, {
    data: bancaChart,
    ref_: piso
  })), /*#__PURE__*/React.createElement(Card, {
    style: {
  ...
  }, "Resultado semanal por jogador"), /*#__PURE__*/React.createElement(MultiBars, {
    data: resChart,
  ...
  }, "Make-up por jogador"), /*#__PURE__*/React.createElement(MultiBars, {
    data: makeChart,
    series: players.map((p, i) => ({
      name: p,
      color: PLAYER_COLORS[i]
  ...
  }, "Volume de torneios por semana"), /*#__PURE__*/React.createElement(MultiBars, {
    data: volChart,
    series: [{
      name: 'Torneios',
      color: C.greenMid
    ...
    }, "Torneios")), /*#__PURE__*/React.createElement("button", {
      onClick: () => addTourOn(today),
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 6,
  ...
  }, "Resumo por jogador e dia \u2014 monta sozinho dos torneios. Toque num dia pra conferir/corrigir."), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 24,
      fontWeight: 600,
  ...
  }, "Di\xE1rio")), /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 16,
      display: 'flex',
      flexDirection: 'column',
  ...
  }), diarySites.length > 1 && /*#__PURE__*/React.createElement(Seg, {
    label: "Plataforma",
    value: diF.site,
    options: diarySites,
    onChange: v => setDiF(f => ({
  ...
  }), diaryMods.length > 1 && /*#__PURE__*/React.createElement(Seg, {
    label: "Modalidade",
    value: diF.mod,
    options: diaryMods,
    onChange: v => setDiF(f => ({
  ...
  }, [7, 15, 30].map(n => /*#__PURE__*/React.createElement("button", {
    key: n,
    onClick: () => setDiF(f => ({
      ...f,
      days: String(f.days) === String(n) ? '' : String(n),
      from: '',
      to: ''
    })),
    style: {
      padding: '6px 11px',
      borderRadius: 99,
      border: `1.5px solid ${String(diF.days) === String(n) ? P : C.border}`,
      background: String(diF.days) === String(n) ? C.plumSoft : C.surface,
      color: String(diF.days) === String(n) ? P : C.inkSoft,
      fontWeight: 700,
      fontSize: 12.5,
      cursor: 'pointer'
    }
  }, n, " dias")), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      color: C.inkSoft
    }
  }, "ou"), /*#__PURE__*/React.createElement("input", {
    type: "date",
    value: diF.from,
    onChange: e => setDiF(f => ({
      ...f,
  ...
  }), diFiltroAtivo && /*#__PURE__*/React.createElement("button", {
    onClick: () => setDiF({
      player: 'Todos',
      site: 'Todos',
      mod: 'Todos',
  ...
  }, "Limpar")))), diFiltroAtivo && /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 18
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "C\xE1lculo do per\xEDodo"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      color: C.inkSoft
    }
  }, diPeriodo ? diPeriodo.label : 'todo o histórico', diF.player !== 'Todos' ? ` · ${diF.player}` : '', diF.site !== 'Todos' ? ` · ${diF.site}` : '', diF.mod !== 'Todos' ? ` · ${diF.mod}` : ''), /*#__PURE__*/React.createElement("span", {
    style: {
      marginLeft: 'auto',
      fontFamily: "'Space Grotesk',sans-serif",
      fontWeight: 700,
      fontSize: 19,
      color: diAgg.res >= 0 ? C.greenMid : C.red
    }
  }, diAgg.res >= 0 ? '+' : '−', fmt(Math.abs(diAgg.res)))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
      gridTemplateColumns: 'repeat(auto-fit,minmax(88px,1fr))',
      gap: 8
    ...
    label: "Dias",
    value: String(diAgg.dias)
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "Torneios",
    value: String(diAgg.torneios)
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "ABI m\xE9dio",
    value: fmt(diAgg.abi)
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "Investido",
    value: fmt(diAgg.buyins)
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "Premia\xE7\xE3o",
    value: fmt(diAgg.premios),
    tone: C.greenMid
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "ROI",
    value: pctFmt(diAgg.roi),
    tone: diAgg.res >= 0 ? C.greenMid : C.red
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "ITM",
  ...
  }, "Geral da pool"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: C.inkSoft,
      marginBottom: 12
    }
  }, "Split ", pctFmt(num(config.player_pct) * 100), " pro jogador \xB7 ", pctFmt((1 - num(config.player_pct)) * 100), " pra pool"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
      gridTemplateColumns: '1fr 1fr',
      gap: 12
  ...
  }, "Lucro acumulado da pool"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 22,
      fontWeight: 600,
  ...
  }, fmt(lucroPoolAcum))), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: 14,
      borderRadius: 14,
      background: C.bg
  ...
  }, "Total pago em saques"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 22,
      fontWeight: 600,
  ...
  }, fmt(totalPago))))), !solo && /*#__PURE__*/React.createElement(SemanalGeral, {
    geral: geralSemana,
    curMonth: curMonthISO
  }), players.map(p => /*#__PURE__*/React.createElement(SemanalPlayer, {
    key: p,
    player: p,
    weeks: weeksByPlayer[p] || [],
    config: config,
    curMonth: curMonthISO,
    makeUpAtual: curMakeUp[p],
    solo: solo
  }))), view === 'mensal' && /*#__PURE__*/React.createElement("div", {
    className: "ftfade",
    style: {
      display: 'flex',
      flexDirection: 'column',
  ...
  }, "Consolidado de"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Space Grotesk',sans-serif",
      fontSize: 26,
      fontWeight: 600,
  ...
  }, mLabel(month))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8
    }
  ...
  }, "\uD83D\uDCC4 Gerar relat\xF3rio do m\xEAs (PDF)"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
      gridTemplateColumns: '1fr 1fr',
      gap: 14
    ...
    tone: C.gold,
    bg: C.goldSoft,
    label: "Torneios no m\xEAs",
    value: String(mTorneios),
    sub: `ABI médio ${fmt(mAbi)}`
  }), /*#__PURE__*/React.createElement(Stat, {
    Icon: IcoPanel,
    tone: mRoi >= 0 ? C.greenMid : C.red,
    bg: mRoi >= 0 ? C.greenSoft : C.redSoft,
    label: "ROI do m\xEAs",
    value: pctFmt(mRoi),
    sub: `sobre ${fmt(mBuyins)} em buy-ins`
  })), /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 20
    }
  ...
  }, "Por jogador"), perPlayerMonth.map(pm => /*#__PURE__*/React.createElement("div", {
    key: pm.p,
    style: {
      padding: '12px 0',
      borderBottom: `1px solid ${C.border}`
  ...
  }, fmt(pm.makeAberto))), /*#__PURE__*/React.createElement("span", {
    style: {
      fontWeight: 800,
  ...
  }, pm.res >= 0 ? '+' : '−', fmt(Math.abs(pm.res)))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
    ...
    label: "ROI",
    value: pctFmt(pm.roi),
    tone: pm.res >= 0 ? C.greenMid : C.red
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "Investido",
    value: fmt(pm.vol)
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "Premia\xE7\xE3o",
    value: fmt(pm.premios),
    tone: C.greenMid
  }), /*#__PURE__*/React.createElement(MiniStat, {
    label: "ITM",
    value: pctFmt(pm.itm)
  ...
  }, "Mais lucrativo"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      marginTop: 2
    }
  }, maisLucrativo ? `${maisLucrativo.p} (${fmt(maisLucrativo.res)})` : '—')), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: 12,
      borderRadius: 12,
      background: C.bg
  ...
  }, "Maior volume"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      marginTop: 2
    }
  }, maiorVolume ? `${maiorVolume.p} (${fmt(maiorVolume.vol)})` : '—')), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: 12,
      borderRadius: 12,
      background: C.greenSoft
  ...
  }, "Melhor dia"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      marginTop: 2
    }
  ...
  }, "Pior dia"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      marginTop: 2
    }
    ...
    value: mensalWho,
    options: ['Geral', ...players],
    onChange: setMensalWho
  })), monthToursWho.length === 0 ? /*#__PURE__*/React.createElement(Empty, null, "Sem torneios de ", mensalWho, " neste m\xEAs.") : [['Por site', bySite], ['Por modalidade', byModality]].map(([titulo, rows]) => /*#__PURE__*/React.createElement("div", {
    key: titulo,
    style: {
      marginBottom: 14
    }
    ...
    const mx = Math.max(1, ...rows.map(x => Math.abs(x.res)));
    return /*#__PURE__*/React.createElement("div", {
      key: g.k,
      style: {
        padding: '8px 0',
    ...
    }, g.n, " torneio", g.n !== 1 ? 's' : '', " \xB7 investido ", fmt(g.inv), " \xB7 ROI ", /*#__PURE__*/React.createElement("b", {
      style: {
        color: g.res >= 0 ? C.greenMid : C.red
      }
    }, pctFmt(g.roi)))), /*#__PURE__*/React.createElement("span", {
      style: {
        fontWeight: 800,
        fontSize: 14.5,
        color: g.res >= 0 ? C.greenMid : C.red,
    ...
    }, g.res >= 0 ? '+' : '−', fmt(Math.abs(g.res)))), /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 6
      }
    }, /*#__PURE__*/React.createElement(Bar2, {
      pct: Math.abs(g.res) / mx * 100,
      color: g.res >= 0 ? C.greenMid : C.red
    })));
  }))))), view === 'stats' && (() => {
    ...
    if (!canStats) return /*#__PURE__*/React.createElement("div", {
      className: "ftfade",
      style: {
        display: 'flex',
        flexDirection: 'column',
    ...
    }, "Estat\xEDsticas de jogo a partir dos hand histories."), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 24,
        fontWeight: 600,
    ...
    }, "Stats")), /*#__PURE__*/React.createElement(Card, {
      style: {
        padding: 26,
        textAlign: 'center',
        maxWidth: 560,
    ...
    }, "Desbloqueie suas estat\xEDsticas com o plano Pro"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 13.5,
        color: C.inkSoft,
        lineHeight: 1.7,
    ...
    }, "\u2713 Importa suas m\xE3os da ", /*#__PURE__*/React.createElement("b", null, "GG (PokerCraft)"), " e do ", /*#__PURE__*/React.createElement("b", null, "PokerStars"), " \u2014 pode mandar o zip", /*#__PURE__*/React.createElement("br", null), "\u2713 VPIP, PFR, 3-bet, c-bet, WWSF, W$SD, AF e ", /*#__PURE__*/React.createElement("b", null, "bb/100 por posi\xE7\xE3o"), /*#__PURE__*/React.createElement("br", null), "\u2713 ", /*#__PURE__*/React.createElement("b", null, "Sorte nos all-ins"), ": quanto do resultado \xE9 jogo e quanto \xE9 vari\xE2ncia", /*#__PURE__*/React.createElement("br", null), "\u2713 ", /*#__PURE__*/React.createElement("b", null, "Leituras autom\xE1ticas"), " em portugu\xEAs comparando com faixas de reg", /*#__PURE__*/React.createElement("br", null), "\u2713 ", /*#__PURE__*/React.createElement("b", null, "Relat\xF3rio completo em PDF"), " pra revisar ou mandar pro coach"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 26,
        fontWeight: 700,
    ...
    }, "R$ 49,90", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 14,
        color: C.inkSoft,
        fontWeight: 600
    ...
    }, "Seu plano atual: ", /*#__PURE__*/React.createElement("b", null, PLAN_LABEL[plan] || plan), " \u2014 a gest\xE3o de banca continua completa e ilimitada.")));
    ...
    const evbb = S('allin_ev_bb'),
      netbb = S('allin_net_bb'),
      sorte = netbb - evbb;
    const afc = S('af_calls'),
      afb = S('af_bets');
    const pos = {};
    rows.forEach(r => {
      const pj = r.pos_json || {};
      Object.keys(pj).forEach(k => {
        ...
        pos[k].v += num(pj[k].v);
        pos[k].p += num(pj[k].p);
        pos[k].net += num(pj[k].net);
        pos[k].hn += num(pj[k].hn);
      });
    });
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "Estat\xEDsticas de jogo a partir dos hand histories \u2014 s\xF3 agregados sobem pro banco."), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 24,
        fontWeight: 600,
    ...
    }, "Importar hand histories"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12.5,
        color: C.inkSoft,
        marginBottom: 10
      }
    }, "Arquivos .txt do PokerStars ou export do PokerCraft (GG) \u2014 pode mandar o .zip direto. Reimportar o mesmo torneio substitui, n\xE3o duplica."), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 10,
        flexWrap: 'wrap',
      ...
      onChange: e => {
        const fs = [...e.target.files];
        e.target.value = '';
        importHH(fs, impPlayer || players[0]);
      }
    }), /*#__PURE__*/React.createElement("label", {
      htmlFor: "hhfile",
      style: {
        ...
        alignItems: 'center',
        gap: 6,
        background: imp.busy ? C.bg : P,
        color: imp.busy ? C.inkSoft : '#fff',
        padding: '11px 16px',
        borderRadius: 13,
        fontWeight: 700,
        fontSize: 14,
        cursor: imp.busy ? 'default' : 'pointer',
        pointerEvents: imp.busy ? 'none' : 'auto'
      }
    }, /*#__PURE__*/React.createElement(IcoPlus, {
      s: 16
    }), imp.busy ? 'Importando…' : 'Escolher arquivos')), imp.busy && /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 10,
        fontSize: 12.5,
        color: P,
    ...
    }, imp.prog), imp.res && /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 10,
        padding: '10px 12px',
        borderRadius: 12,
        background: imp.res.saved || imp.res.repetidas ? C.greenSoft : C.redSoft,
        fontSize: 12.5,
        color: C.ink
      }
    }, /*#__PURE__*/React.createElement("b", null, imp.res.saved, " torneio", imp.res.saved !== 1 ? 's' : '', " salvo", imp.res.saved !== 1 ? 's' : ''), " \xB7 ", imp.res.novas, " m\xE3o", imp.res.novas !== 1 ? 's' : '', " nova", imp.res.novas !== 1 ? 's' : '', " de ", imp.res.files, " arquivo", imp.res.files !== 1 ? 's' : '', imp.res.repetidas ? ` · ${imp.res.repetidas} já importada${imp.res.repetidas !== 1 ? 's' : ''} (pulada${imp.res.repetidas !== 1 ? 's' : ''})` : '', imp.res.reproc ? ` · ${imp.res.reproc} reprocessada${imp.res.reproc !== 1 ? 's' : ''} (bb/100 por posição preenchido)` : '', imp.res.ignored ? ` · ${imp.res.ignored} mão(s) ignorada(s)${(r => {
      const M = {
        cash: 'cash game — só torneios entram',
        modalidade: "outra modalidade — só Hold'em",
        incompleta: 'incompletas',
        formato: 'formato não reconhecido'
      };
      const ps = Object.keys(M).filter(k => r[k]).map(k => `${r[k]} ${M[k]}`);
      return ps.length ? ` — ${ps.join(' · ')}` : '';
    })(imp.res.reasons || {})}` : '', imp.res.issues.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: 6,
        color: C.red
      }
    }, imp.res.issues.map((x, i) => /*#__PURE__*/React.createElement("div", {
      key: i
    }, "\u2022 ", x)), imp.res.issuesTotal > imp.res.issues.length && /*#__PURE__*/React.createElement("div", null, "\u2026 e mais ", imp.res.issuesTotal - imp.res.issues.length))), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11.5,
        color: C.inkSoft,
        marginTop: 10,
    ...
    }, "\u2139\uFE0F ", /*#__PURE__*/React.createElement("b", null, "PokerStars"), ": funciona tanto o hist\xF3rico local (Configura\xE7\xF5es \u2192 Hist\xF3rico de M\xE3os) quanto o ", /*#__PURE__*/React.createElement("b", null, "transcript pedido por e-mail"), ", em portugu\xEAs ou ingl\xEAs. ", /*#__PURE__*/React.createElement("b", null, "GG"), ": exporte o ", /*#__PURE__*/React.createElement("b", null, "hand history"), " pelo PokerCraft (pode mandar o .zip direto). Import grande \xE9 melhor no computador. ", /*#__PURE__*/React.createElement("span", {
      style: {
        opacity: .6
      }
    }, "build ", HH_BUILD))), /*#__PURE__*/React.createElement(Card, {
      style: {
        padding: 16,
        display: 'flex',
        gap: 14,
    ...
    }, [['', 'Tudo'], ['30', '30 dias'], ['90', '90 dias'], ['365', '1 ano']].map(([v, l]) => {
      const ativo = stF.days === v && !rangeAtivo;
      return /*#__PURE__*/React.createElement("button", {
        key: l,
        onClick: () => setStF(f => ({
          ...f,
        ...
        style: {
          padding: '6px 11px',
          borderRadius: 99,
          border: `1.5px solid ${ativo ? P : C.border}`,
          background: ativo ? C.plumSoft : C.surface,
          color: ativo ? P : C.inkSoft,
          fontWeight: 700,
          fontSize: 12.5,
          cursor: 'pointer'
        }
      ...
      style: {
        padding: '6px 9px',
        borderRadius: 10,
        border: `1.5px solid ${stF.from ? P : C.border}`,
        fontSize: 12.5,
        color: C.ink,
        background: C.surface
      }
      ...
      style: {
        padding: '6px 9px',
        borderRadius: 10,
        border: `1.5px solid ${stF.to ? P : C.border}`,
        fontSize: 12.5,
        color: C.ink,
        background: C.surface
      }
    }), rangeAtivo && /*#__PURE__*/React.createElement("button", {
      onClick: () => setStF(f => ({
        ...f,
        days: '',
        from: '',
    ...
    }, "\uD83D\uDCC4 Relat\xF3rio do jogador (PDF)")), rows.length === 0 ? /*#__PURE__*/React.createElement(Empty, null, "Sem dados de ", sp, " nesse per\xEDodo. Importe os hand histories acima.") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'grid',
    ...
    }, "M\xC3OS"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 22,
        fontWeight: 600
    ...
    }, "BB/100"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 22,
        fontWeight: 600,
    ...
    }, "SORTE (ALL-IN)"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 22,
        fontWeight: 600,
    ...
    }, "real ", fmtBB(netbb), " vs EV ", fmtBB(evbb), " \xB7 ", S('allin_cnt'), " all-in", S('allin_cnt') !== 1 ? 's' : '')), /*#__PURE__*/React.createElement(Card, {
      style: {
        padding: 14
      }
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "AGRESS\xC3O (AF)"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 22,
        fontWeight: 600
      }
    }, afc > 0 ? (afb / afc).toFixed(1).replace('.', ',') : '—'), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: C.inkSoft
      }
      ...
      const coachTxt = () => [`Análise de poker — ${sp} (${solo ? 'GrinderBank' : 'PoolGG'})`, `Período: ${periodoLabel} · ${hands} mãos · ${rows.length} torneios`, `bb/100 (fichas): ${fmtBB(bb100)}`, `Sorte all-in: ${fmtBB(sorte)} (real ${fmtBB(netbb)} vs EV ${fmtBB(evbb)}) em ${S('allin_cnt')} all-ins`, `VPIP ${pctS(S('vpip_cnt'), hands)} · PFR ${pctS(S('pfr_cnt'), hands)}`, `3-bet ${pctS(S('tb_cnt'), S('tb_opp'))} · Fold pra 3-bet ${pctS(S('f3b_cnt'), S('f3b_opp'))}`, `Roubo de blinds ${pctS(S('steal_cnt'), S('steal_opp'))} · BB defende ${pctS(S('bbdef_cnt'), S('bbdef_opp'))}`, `C-bet flop ${pctS(S('cbet_cnt'), S('cbet_opp'))} · Fold pra c-bet ${pctS(S('fcbet_cnt'), S('fcbet_opp'))}`, `WWSF ${pctS(S('wwsf_cnt'), S('sawflop_cnt'))} · WTSD ${pctS(S('wtsd_cnt'), S('sawflop_cnt'))} · W$SD ${pctS(S('wsd_cnt'), S('wtsd_cnt'))}`, 'Definições: WWSF e WTSD = % sobre flops vistos; W$SD = % sobre showdowns; bb/100 em fichas; sorte = all-in EV (só cartas viradas, PKO/ICM fora).', `AF: ${afc > 0 ? (afb / afc).toFixed(2).replace('.', ',') : '—'}`, 'Por posição (mãos · VPIP · PFR · bb/100): ' + POS_ORDER.filter(k => pos[k] && pos[k].h).map(k => `${k} ${pos[k].h}·${(pos[k].v / pos[k].h * 100).toFixed(0)}%·${(pos[k].p / pos[k].h * 100).toFixed(0)}%·${pos[k].hn > 0 ? (pos[k].net / pos[k].hn * 100).toFixed(1) : '—'}`).join(' | '), 'Leituras automáticas: ' + (insights.map(i => i.t).join('; ') || '—'), '', `Contexto: jogador de MTT micro/low ${solo ? 'com banca própria' : 'numa pool 50/50 com make-up'}. Analise como coach: os 3 maiores vazamentos, o que treinar primeiro e o impacto esperado no winrate.`].join('\n');
      const copiar = async () => {
        const tx = coachTxt();
        try {
      ...
      const worst = insights[0] ? TONE[insights[0].tone] : {
        c: C.inkSoft,
        bg: C.bg
      };
      return /*#__PURE__*/React.createElement(Card, {
      ...
      }, insights.length, " leitura", insights.length !== 1 ? 's' : ''), /*#__PURE__*/React.createElement("span", {
        style: {
          color: C.inkSoft,
          fontSize: 20,
          transform: leitOpen ? 'rotate(90deg)' : 'none',
          transition: 'transform .2s'
        }
      }, "\u203A")), leitOpen && /*#__PURE__*/React.createElement("div", {
        style: {
          padding: '0 18px 16px'
        }
      }, /*#__PURE__*/React.createElement("div", {
      ...
      }, "Compara\xE7\xE3o autom\xE1tica com as faixas de um reg de MTT micro/low. Pra an\xE1lise profunda, copie o resumo e cole no seu projeto ", /*#__PURE__*/React.createElement("b", null, "Coach de Poker"), " no Claude."), /*#__PURE__*/React.createElement("button", {
        onClick: copiar,
        style: {
          display: 'inline-flex',
          alignItems: 'center',
          ...
          padding: '8px 13px',
          borderRadius: 11,
          border: `1.5px solid ${copied ? C.greenMid : P}`,
          background: copied ? C.greenSoft : C.plumSoft,
          color: copied ? C.greenMid : P,
          fontWeight: 700,
          fontSize: 12.5,
          cursor: 'pointer'
        }
      }, copied ? '✓ Copiado!' : 'Copiar resumo pro Coach')), insights.map((i, ix) => /*#__PURE__*/React.createElement("div", {
        key: ix,
        style: {
          display: 'flex',
          gap: 10,
    ...
    }, "WWSF e WTSD = % sobre os ", /*#__PURE__*/React.createElement("b", null, "flops vistos"), " \xB7 W$SD = % sobre os ", /*#__PURE__*/React.createElement("b", null, "showdowns"), " (o HUD da GG usa outra base \u2014 detalhe no gloss\xE1rio). Stat com ", /*#__PURE__*/React.createElement("b", null, "\u26A0"), " tem uma leitura \u2014 toque no n\xFAmero pra ver.")), /*#__PURE__*/React.createElement(Card, {
      style: {
        padding: 18
      }
    }, /*#__PURE__*/React.createElement("h3", {
    ...
    }, "Sorte nos all-ins \u2014 como ler esse n\xFAmero"), /*#__PURE__*/React.createElement("span", {
      style: {
        padding: '3px 10px',
        borderRadius: 99,
        fontSize: 11,
    ...
    }, fmtBB(sorte)), /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.inkSoft,
        fontSize: 20,
        transform: sorteOpen ? 'rotate(90deg)' : 'none',
        transition: 'transform .2s'
      }
    }, "\u203A")), sorteOpen && /*#__PURE__*/React.createElement("div", {
      style: {
        padding: '0 18px 16px'
      }
    }, (() => {
      ...
      return /*#__PURE__*/React.createElement("div", {
        style: {
          padding: '12px 14px',
          borderRadius: 12,
          background: neutro ? C.bg : sorte >= 0 ? C.greenSoft : C.redSoft,
          marginBottom: 12
        }
      }, /*#__PURE__*/React.createElement("div", {
        style: {
      ...
      }, "Veredito do per\xEDodo \xB7 ", periodoLabel, " \xB7 ", hands, " m\xE3o", hands !== 1 ? 's' : '', " \xB7 ", allins, " all-in", allins !== 1 ? 's' : ''), allins === 0 ? /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 13.5,
          color: C.inkSoft,
          marginTop: 4
    ...
    }, "O que significa cada estat\xEDstica?"), /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.inkSoft,
        fontSize: 20,
        transform: gloss ? 'rotate(90deg)' : 'none',
        transition: 'transform .2s'
      }
    }, "\u203A")), gloss && /*#__PURE__*/React.createElement("div", {
      style: {
        padding: '0 18px 16px'
      }
    }, HH_GLOSS.map(([k, v]) => /*#__PURE__*/React.createElement("div", {
      key: k,
      style: {
        padding: '9px 0',
        borderBottom: `1px solid ${C.border}`
    ...
    for (const r of hhr) {
      ...
      const pick = list => {
        let best = null,
          bd = 1e9;
        for (const t of list) {
          const df = Math.abs(num(t.buyin) - b);
          if (df < bd) {
            bd = df;
            best = t;
          }
      ...
      if (m && (near2(num(m.buyin), b) || close2(num(m.buyin), b))) {
        usados.add(m.id);
        linhas.push({
          tipo: near2(num(m.buyin), b) ? 'ok' : 'buyin',
          hh: r,
          t: m,
          date: r.entry_date
        });
    ...
    decl.filter(t => !usados.has(t.id)).forEach(t => linhas.push({
      tipo: 'semhh',
      t,
      date: t.entry_date
    }));
    ...
    const veredito = !temDado ? {
      tone: C.inkSoft,
      bg: C.bg,
      t: 'Sem dados no período',
      x: 'Lance torneios e importe hand histories pra auditoria comparar os dois.'
    } : n('semlanc') || n('buyin') ? {
      tone: C.red,
      bg: C.redSoft,
      t: 'Divergências encontradas',
      x: 'Tem diferença entre o que as mãos provam e o que foi lançado — confira as linhas em vermelho/dourado abaixo.'
    } : n('semhh') ? {
      tone: C.gold,
      bg: C.goldSoft,
      t: 'Batendo, mas falta comprovação',
      x: 'Nenhuma divergência, porém há torneios lançados sem hand history importado — importe as mãos pra fechar a prova.'
    } : {
      tone: C.greenMid,
      bg: C.greenSoft,
      t: 'Tudo batendo',
      x: 'Cada torneio lançado tem hand history correspondente com o mesmo buy-in. Prova fechada.'
    };
    const TIPO = {
      ok: {
        c: C.greenMid,
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "O que as m\xE3os provam vs. o que foi lan\xE7ado \u2014 a prova de que o controle da pool est\xE1 honesto."), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 24,
        fontWeight: 600,
    ...
    }, "Auditoria")), /*#__PURE__*/React.createElement(Card, {
      style: {
        padding: 16,
        display: 'flex',
        gap: 14,
      ...
      value: sp,
      options: players,
      onChange: v => setAudF(f => ({
        ...f,
        player: v
      }))
    }), /*#__PURE__*/React.createElement(Seg, {
      label: "Per\xEDodo",
      value: audF.days === 7 ? '7 dias' : audF.days === 30 ? '30 dias' : audF.days === 90 ? '90 dias' : 'Tudo',
      options: ['7 dias', '30 dias', '90 dias', 'Tudo'],
      onChange: v => setAudF(f => ({
        ...f,
        days: v === '7 dias' ? 7 : v === '30 dias' ? 30 : v === '90 dias' ? 90 : 0
      }))
    })), /*#__PURE__*/React.createElement(Card, {
      style: {
        padding: 18,
        background: veredito.bg,
        border: `1.5px solid ${veredito.tone}22`
      }
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
    ...
    }, veredito.t), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12.5,
        color: C.ink,
        lineHeight: 1.55,
    ...
    }, veredito.x)))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'grid',
        gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))',
        gap: 10
      }
    }, [['Torneios lançados', decl.length, C.ink], ['Com prova (mãos)', n('ok') + n('buyin'), C.greenMid], ['Jogados sem lançar', n('semlanc'), n('semlanc') ? C.red : C.inkSoft], ['Lançados sem mãos', n('semhh'), n('semhh') ? C.gold : C.inkSoft]].map(([l, v, c]) => /*#__PURE__*/React.createElement(Card, {
      key: l,
      style: {
        padding: '14px 16px'
      }
    ...
    }, v)))), foraHH.length > 0 && /*#__PURE__*/React.createElement(Card, {
      style: {
        padding: 16,
        background: C.redSoft
      }
    ...
    }, "\u26A0\uFE0F Fora da grade COMPROVADO pelas m\xE3os (", foraHH.length, ")"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12.5,
        color: C.ink,
        lineHeight: 1.5
      }
    }, foraHH.map(r => `${r.tournament_name || '#' + r.site_tournament_id} · ${dLabel(r.entry_date)} · buy-in ${fmt(r.buyin)} (máx da época ${fmt(abiMaxFor(config, sp, r.entry_date))})`).join(' — '))), /*#__PURE__*/React.createElement(Card, {
      style: {
        padding: 18
      }
    }, /*#__PURE__*/React.createElement("h3", {
    ...
    }, "Reconcilia\xE7\xE3o torneio a torneio"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12,
        color: C.inkSoft,
        marginBottom: 10
      }
    }, "Cada linha casa um torneio do hand history com um lan\xE7amento (mesmo site, mesmo dia \u2014 ou vizinho \u2014 e buy-in igual; at\xE9 15% de diferen\xE7a conta como \"quase\", acima disso n\xE3o casa)."), !linhas.length && /*#__PURE__*/React.createElement(Empty, null, "Nada pra reconciliar no per\xEDodo."), linhas.map((x, i) => /*#__PURE__*/React.createElement("div", {
      key: i,
      style: {
        display: 'flex',
        gap: 10,
      ...
      style: {
        ...
        fontWeight: 800,
        color: TIPO[x.tipo].c,
        background: TIPO[x.tipo].bg,
        whiteSpace: 'nowrap',
        flexShrink: 0,
        marginTop: 1
      }
    }, TIPO[x.tipo].l), /*#__PURE__*/React.createElement("div", {
      style: {
        minWidth: 0,
        flex: 1
      }
    ...
    }, dLabel(x.date), " \xB7 ", x.hh && (x.hh.tournament_name || '#' + x.hh.site_tournament_id) || x.t && (x.t.tournament_name || 'Torneio'), " ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.inkSoft,
        fontWeight: 600
      }
    }, "\xB7 ", x.hh && x.hh.site || x.t && x.t.site || '')), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12,
        color: C.inkSoft,
        marginTop: 1
      }
    }, x.tipo === 'ok' && `Lançado ${fmt(x.t.buyin)} · mãos comprovam ${fmt(x.hh.buyin)} · ${x.hh.hands} mão(s) no arquivo`, x.tipo === 'buyin' && `Lançado ${fmt(x.t.buyin)} mas as mãos mostram ${fmt(x.hh.buyin)} — confere qual está certo e corrige o lançamento`, x.tipo === 'semlanc' && `As mãos provam que esse torneio foi jogado (${x.hh.hands} mão(s), buy-in ${fmt(x.hh.buyin)}) mas não há lançamento correspondente na aba Torneios`, x.tipo === 'semhh' && `Lançado ${fmt(x.t.buyin)}${num(x.t.prize) > 0 ? ` · prêmio ${fmt(x.t.prize)}` : ''} — sem hand history importado pra comprovar (importe as mãos na aba Stats)`))))));
  })(), view === 'saques' && /*#__PURE__*/React.createElement("div", {
    className: "ftfade",
    style: {
      display: 'flex',
      flexDirection: 'column',
  ...
  }, "Saques por semana"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: C.inkSoft,
      marginBottom: 10
    }
  }, "O valor autorizado vem do c\xE1lculo semanal. Registre aqui s\xF3 o que foi ", /*#__PURE__*/React.createElement("b", null, "realmente pago"), " \u2014 o valor sai da banca sozinho."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap',
  ...
  }), p, " tem ", fmt(aReceber[p]), " a receber"))), (() => {
    const rows = players.flatMap(p => (weeksByPlayer[p] || []).map(w => ({
      w,
      p,
      vs: valorSacadoFor(w.week, p)
    }))).filter(x => x.w.saqueAut > 0 || x.vs > 0).sort((a, b) => a.w.week < b.w.week ? 1 : -1);
    return rows.length ? rows.map(({
      w,
      p,
      vs
    }) => {
      const st = saqueStatus(w, bancaAtual, piso, vs);
      const podePagar = st === 'Autorizado' || st === 'Parcialmente autorizado';
      return /*#__PURE__*/React.createElement("div", {
        key: w.week + p,
        style: {
          display: 'flex',
      ...
      }, p, " \xB7 semana at\xE9 ", dLabel(w.week)), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 12.5,
          color: C.inkSoft,
          marginTop: 2
        }
      }, "autorizado ", fmt(w.saqueAut), " \xB7 pago ", fmt(vs), " ", /*#__PURE__*/React.createElement(Badge, {
        text: st
      }))), podePagar && /*#__PURE__*/React.createElement("button", {
        onClick: () => setModal({
          type: 'wd',
          initial: {
            week_ending_date: w.week,
            player: p,
            valor_sacado: String(Math.max(0, w.saqueAut - vs)).replace('.', ',')
          }
        }),
        style: {
          padding: '8px 14px',
    ...
    }) : /*#__PURE__*/React.createElement(Empty, null, "Nenhuma semana com saque autorizado ainda.");
  })()), /*#__PURE__*/React.createElement(ListView, {
    title: "Pagamentos registrados",
    onAdd: () => setModal('wd'),
    rows: [...wds].sort((a, b) => a.week_ending_date < b.week_ending_date ? 1 : -1),
    empty: "Nenhum pagamento registrado.",
    renderRow: w => /*#__PURE__*/React.createElement(Row, {
      key: w.id,
      ...
      left: /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
        style: {
          ...
          placeItems: 'center',
          flexShrink: 0,
          color: pcolor(w.player)
        }
      }, /*#__PURE__*/React.createElement(IcoCashOut, {
        s: 19
      })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        style: {
          fontWeight: 700,
      ...
      }, w.player), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 12.5,
          color: C.inkSoft
        }
      }, "semana at\xE9 ", dLabel(w.week_ending_date), w.observacoes ? ` · ${w.observacoes}` : ''))),
      right: /*#__PURE__*/React.createElement("span", {
        style: {
          fontWeight: 800,
          fontSize: 16,
  ...
  })), view === 'banca' && /*#__PURE__*/React.createElement("div", {
    className: "ftfade",
    style: {
      display: 'flex',
      flexDirection: 'column',
  ...
  }, "Banca inicial"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      fontSize: 15
    }
  }, fmt(config.banca_inicial))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12.5,
      opacity: .85
    }
  }, "Resultado dos jogos"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      fontSize: 15
    }
  }, resultadosTotal >= 0 ? '+' : '−', fmt(Math.abs(resultadosTotal)))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12.5,
      opacity: .85
    }
  }, "Aportes/movimentos"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      fontSize: 15
    }
  }, movimentosTotal >= 0 ? '+' : '−', fmt(Math.abs(movimentosTotal)))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12.5,
      opacity: .85
    }
  }, "Saques pagos"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      fontSize: 15
    }
  }, "\u2212", fmt(sacadoTotal))))), /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 20
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Por jogador e plataforma"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12.5,
      color: C.inkSoft,
      marginTop: 2
    }
  }, "Onde o dinheiro est\xE1. Atualiza sozinho com torneios, saques e movimentos.")), /*#__PURE__*/React.createElement("button", {
    ...
    s: 16
  }), "Transferir")), players.map((p, i) => {
    const wl = walletsDoJogador(p);
    const tot = saldoJogador(p);
    return /*#__PURE__*/React.createElement("div", {
      key: p,
      style: {
        padding: '12px 0',
      ...
      const wr = wl.find(x => x.wallet === w);
      return /*#__PURE__*/React.createElement("button", {
        key: w,
        onClick: () => setQuickEdit({
          label: `Saldo inicial · ${p} · ${w}`,
          hint: 'O que o jogador tinha nessa plataforma no começo. O app soma torneios, movimentos e saques em cima disso.',
          kind: 'money',
          current: walletInicial(p, w),
          onSave: nv => saveWallet(p, w, nv)
        }),
        style: {
          display: 'flex',
          flexDirection: 'column',
          ...
          borderRadius: 11,
          border: `1px solid ${C.border}`,
          background: wr ? C.warm : 'transparent',
          cursor: 'pointer',
          minWidth: 96
        }
      }, /*#__PURE__*/React.createElement("span", {
        ...
        style: {
          ...
          fontSize: 15,
          fontWeight: 600,
          color: wr ? wr.saldo >= 0 ? C.ink : C.red : '#C3BBA9'
        }
      }, wr ? fmt(wr.saldo) : 'definir'));
    })));
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
  ...
  }, "Toque numa plataforma pra ajustar o ", /*#__PURE__*/React.createElement("b", null, "saldo inicial"), " daquele jogador. Dep\xF3sitos/transfer\xEAncias entram como ", /*#__PURE__*/React.createElement("b", null, "movimento"), " abaixo (escolhendo o jogador); saque pago sai da carteira escolhida na aba Saques."), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11.5,
      color: C.inkSoft,
      marginTop: 8,
  ...
  }, "\u2139\uFE0F Esta \xE9 a vis\xE3o de ", /*#__PURE__*/React.createElement("b", null, "quem tem o qu\xEA"), ", montada a partir do saldo inicial que cada um declarou por plataforma. Ela ", /*#__PURE__*/React.createElement("b", null, "n\xE3o precisa bater"), " com o total da ", /*#__PURE__*/React.createElement("b", null, "banca central"), " l\xE1 em cima \u2014 a banca central parte da banca inicial da pool, n\xE3o desses saldos por jogador. Torneio/movimento/saque num site fora das plataformas listadas cai em ", /*#__PURE__*/React.createElement("b", null, "\"Outros\""), ".")), /*#__PURE__*/React.createElement(ListView, {
    title: "Movimentos da banca",
    subtitle: "S\xF3 aportes e retiradas externas (dinheiro entrando ou saindo da pool). Resultado de jogo entra sozinho pelo Di\xE1rio, e saque de jogador pela aba Saques \u2014 n\xE3o registre eles aqui de novo.",
    onAdd: () => setModal('bank'),
    rows: [...ledger].sort((a, b) => a.entry_date < b.entry_date ? 1 : -1),
    empty: "Nenhum movimento na banca.",
    renderRow: l => {
      const net = num(l.entrada) - num(l.saida);
      return /*#__PURE__*/React.createElement(Row, {
        key: l.id,
        ...
        left: /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
          style: {
            ...
            height: 40,
            borderRadius: 12,
            background: net >= 0 ? C.greenSoft : C.redSoft,
            display: 'grid',
            placeItems: 'center',
            flexShrink: 0,
            color: net >= 0 ? C.greenMid : C.red
          }
        }, net >= 0 ? /*#__PURE__*/React.createElement(IcoUp, {
          s: 20
        }) : /*#__PURE__*/React.createElement(IcoDown, {
          s: 20
        })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
          style: {
            fontWeight: 700,
        ...
        }, l.wallet, l.player ? ` · ${l.player}` : ''), /*#__PURE__*/React.createElement("div", {
          style: {
            fontSize: 12.5,
            color: C.inkSoft
          }
        }, dLabel(l.entry_date), l.observacao ? ` · ${l.observacao}` : ''))),
        right: /*#__PURE__*/React.createElement("span", {
          style: {
            fontWeight: 800,
            fontSize: 16,
  ...
  })), view === 'config' && /*#__PURE__*/React.createElement("div", {
    className: "ftfade"
  }, /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 20
  ...
  }, "Configura\xE7\xF5es da pool"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: C.inkSoft,
      marginBottom: 12
    }
  }, "Toque no l\xE1pis pra ajustar. Todos veem a mesma configura\xE7\xE3o."), (solo ? [{
    label: 'Seu nome',
    val: config.player1_name,
    kind: 'text',
    key: 'player1_name'
  }, {
    label: 'ABI máximo',
    val: config.abi_max_player1,
    kind: 'money',
    key: 'abi_max_player1',
    show: fmt(config.abi_max_player1)
  }, {
    label: 'Stop loss diário (buy-ins)',
    val: config.stoploss_daily_buyins,
    kind: 'money',
    key: 'stoploss_daily_buyins',
    show: String(num(config.stoploss_daily_buyins))
  }, {
    label: 'Stop loss semanal',
    val: config.stoploss_weekly_pct,
    kind: 'percent',
    key: 'stoploss_weekly_pct',
    show: pctFmt(num(config.stoploss_weekly_pct) * 100)
  }, {
    label: 'Banca inicial',
    val: config.banca_inicial,
    kind: 'money',
    key: 'banca_inicial',
    show: fmt(config.banca_inicial)
  }, {
    label: 'Início da semana de referência',
    val: config.week_start_date,
    kind: 'date',
    key: 'week_start_date',
    show: config.week_start_date
  }, {
    label: 'Sites permitidos',
    val: config.sites_permitidos,
    kind: 'list',
    key: 'sites_permitidos',
    show: sites.join(', ')
  }, {
    label: 'Modalidades permitidas',
    val: config.modalidades_permitidas,
    kind: 'list',
    key: 'modalidades_permitidas',
    show: modalidades.join(', ')
  }] : [{
    label: 'Jogador 1',
    val: config.player1_name,
    kind: 'text',
    ...
    label: 'Jogador 2',
    val: config.player2_name,
    kind: 'text',
    key: 'player2_name'
  }, {
    label: 'Fatia do jogador no lucro',
    val: config.player_pct,
    kind: 'percent',
    key: 'player_pct',
    show: pctFmt(num(config.player_pct) * 100)
  }, {
    label: `ABI máximo · ${config.player1_name}`,
    val: config.abi_max_player1,
    kind: 'money',
    key: 'abi_max_player1',
    show: fmt(config.abi_max_player1)
  }, {
    label: `ABI máximo · ${config.player2_name}`,
    val: config.abi_max_player2,
    kind: 'money',
    key: 'abi_max_player2',
    show: fmt(config.abi_max_player2)
  }, {
    label: 'Piso mínimo da banca',
    val: config.piso_minimo,
    kind: 'money',
    key: 'piso_minimo',
    show: fmt(config.piso_minimo)
  }, {
    label: 'Make-up máx. recomendado',
    val: config.makeup_max_recomendado,
    kind: 'money',
    key: 'makeup_max_recomendado',
    show: fmt(config.makeup_max_recomendado)
  }, {
    label: `Make-up inicial · ${config.player1_name}`,
    val: config.makeup_inicial_player1,
    kind: 'money',
    key: 'makeup_inicial_player1',
    show: fmt(config.makeup_inicial_player1)
  }, {
    label: `Make-up inicial · ${config.player2_name}`,
    val: config.makeup_inicial_player2,
    kind: 'money',
    key: 'makeup_inicial_player2',
    show: fmt(config.makeup_inicial_player2)
  }, {
    label: 'Stop loss diário (buy-ins)',
    val: config.stoploss_daily_buyins,
    kind: 'money',
  ...
  }, row.show != null ? row.show : String(row.val))), /*#__PURE__*/React.createElement("button", {
    onClick: () => setQuickEdit({
      label: row.label,
      kind: row.kind,
      current: row.val,
      onSave: nv => askSaveConfig(row, nv)
    }),
    style: {
      width: 38,
      height: 38,
  ...
  }, "Hist\xF3rico de altera\xE7\xF5es"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: C.inkSoft,
      marginBottom: 10
    }
  }, "Toda mudan\xE7a nos Ajustes fica registrada aqui e o outro jogador \xE9 avisado na hora. A grade (\"fora da grade\") julga cada torneio pelo ABI que valia ", /*#__PURE__*/React.createElement("b", null, "na data dele"), "."), chgs.length === 0 && /*#__PURE__*/React.createElement(Empty, null, "Nenhuma altera\xE7\xE3o registrada ainda."), chgs.slice(0, 12).map(c => /*#__PURE__*/React.createElement("div", {
    key: c.id,
    style: {
      padding: '10px 0',
      borderBottom: `1px solid ${C.border}`
  ...
  }, c.changed_by_name || '—', " \xB7 ", dLabel(String(c.created_at).slice(0, 10)))))), solo && /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 20,
      marginTop: 16
    }
  ...
  }, "Plano atual: ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: P
    }
  ...
  }, "Rever o tour do sistema")), /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 20,
      marginTop: 16
    }
  ...
  }, "Quer sair de vez? Voc\xEA pode ", /*#__PURE__*/React.createElement("b", null, "excluir sua conta"), " e apagar todos os seus dados definitivamente (direito garantido pela LGPD)."), /*#__PURE__*/React.createElement("button", {
    ...
    s: 17
  }), "Excluir minha conta"))), /*#__PURE__*/React.createElement(Card, {
    style: {
      padding: 20,
      marginTop: 16
    }
  ...
  }, "Backup (CSV)"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: C.inkSoft,
      marginBottom: 14
    }
  }, "Baixa os dados brutos pra guardar. Abre direto no Excel/Planilhas."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap'
    }
  }, [['Diário', daily, 'diario'], ['Torneios', tours, 'torneios'], ['Banca', ledger, 'banca'], ['Saques', wds, 'saques'], ['Configurações', config ? [config] : [], 'config']].map(([label, rows, file]) => /*#__PURE__*/React.createElement("button", {
    key: file,
    disabled: !rows.length,
    onClick: () => downloadCSV(`pool-${file}`, rows),
    style: {
      padding: '10px 16px',
      borderRadius: 12,
      border: `1px solid ${C.border}`,
      background: rows.length ? C.surface : C.bg,
      color: rows.length ? P : '#C3BBA9',
      fontWeight: 700,
      fontSize: 13.5,
      cursor: rows.length ? 'pointer' : 'default'
    }
  }, label, " (", rows.length, ")")))))), /*#__PURE__*/React.createElement("nav", {
    className: "bottomnav",
    style: {
      position: 'fixed',
      bottom: 0,
      ...
      gap: 2,
      boxShadow: '0 -8px 24px -16px rgba(0,0,0,.25)',
      zIndex: 20
    }
  }, nav.filter(n => MAIN_TABS.includes(n.id)).map(n => /*#__PURE__*/React.createElement(NavBtn, {
    key: n.id,
    Icon: n.Icon,
    label: n.label,
    active: view === n.id,
    ...
    Icon: IcoMenu,
    label: "Mais",
    active: menuOpen || !MAIN_TABS.includes(view),
  ...
  }, "Todas as telas"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setMenuOpen(false),
    style: {
      width: 38,
      height: 38,
  ...
  }, nav.filter(n => !MAIN_TABS.includes(n.id)).map(n => /*#__PURE__*/React.createElement("button", {
    key: n.id,
    onClick: () => {
      setView(n.id);
      setMenuOpen(false);
    ...
    style: {
      ...
      padding: '16px 14px',
      borderRadius: 14,
      border: `1.5px solid ${view === n.id ? P : C.border}`,
      background: view === n.id ? C.plumSoft : C.warm,
      color: view === n.id ? P : C.ink,
      fontWeight: 700,
      fontSize: 15,
      cursor: 'pointer'
    }
    ...
    }, solo ? 'GrinderBank' : 'PoolGG'), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 12,
        color: '#6B6455'
      }
    }, solo ? `banca de ${config.player1_name}` : `pool de poker · ${config.player1_name} & ${config.player2_name}`)), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 19,
        fontWeight: 600,
    ...
    }, sub, " \xB7 gerado em ", dLabel(hoje), "/", hoje.slice(0, 4)));
    const Big = ({
      label,
      value,
      tone
    ...
    if (report.type === 'mensal') {
      ...
      const wkRows = players.flatMap(pl => (weeksByPlayer[pl] || []).filter(w => w.week.slice(0, 7) === month).map(w => ({
        ...w,
        pl,
        vs: valorSacadoFor(w.week, pl)
      })));
      const mSaques = wds.filter(w => String(w.week_ending_date || '').slice(0, 7) === month);
      const bs = breakdownBy('site', monthTours),
        bm = breakdownBy('modality', monthTours);
      corpo = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Sec, {
        t: "Resumo do m\xEAs"
      }, /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'grid',
      ...
      }, "Sem semanas fechadas neste m\xEAs.")), mSaques.length > 0 && /*#__PURE__*/React.createElement(Sec, {
        t: "Saques pagos no m\xEAs"
      }, /*#__PURE__*/React.createElement("table", {
        className: "reptable"
      }, /*#__PURE__*/React.createElement("thead", null, /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("th", null, "Semana"), /*#__PURE__*/React.createElement("th", null, "Jogador"), /*#__PURE__*/React.createElement("th", null, "Carteira"), /*#__PURE__*/React.createElement("th", null, "Valor"))), /*#__PURE__*/React.createElement("tbody", null, mSaques.map(w => /*#__PURE__*/React.createElement("tr", {
        key: w.id
      }, /*#__PURE__*/React.createElement("td", null, dLabel(w.week_ending_date)), /*#__PURE__*/React.createElement("td", null, w.player), /*#__PURE__*/React.createElement("td", null, w.wallet || '—'), /*#__PURE__*/React.createElement("td", {
        style: {
          fontWeight: 700
        }
      ...
      }, [['Por site', bs], ['Por modalidade', bm]].map(([tt, rws]) => /*#__PURE__*/React.createElement("div", {
        key: tt
      }, /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 11,
      ...
      }, tt), /*#__PURE__*/React.createElement("table", {
        className: "reptable"
      }, /*#__PURE__*/React.createElement("tbody", null, rws.map(g => /*#__PURE__*/React.createElement("tr", {
        key: g.k
      }, /*#__PURE__*/React.createElement("td", null, g.k), /*#__PURE__*/React.createElement("td", null, g.n, " tor."), /*#__PURE__*/React.createElement("td", null, pctFmt(g.roi)), /*#__PURE__*/React.createElement("td", {
        style: {
          color: g.res >= 0 ? C.greenMid : C.red,
          fontWeight: 700
        }
      }, fmt(g.res)))))))))), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 11,
          color: '#6B6455'
        }
      ...
      const pDayRes = {};
      pT.forEach(t => {
        pDayRes[t.entry_date] = (pDayRes[t.entry_date] || 0) + lucroTorneio(t);
      });
      ...
      const hands2 = S2('hands'),
        bb100v = hands2 > 0 ? S2('net_bb') / hands2 * 100 : 0;
      const evbb = S2('allin_ev_bb'),
        netbb = S2('allin_net_bb'),
        sorte = netbb - evbb;
      const afb = S2('af_bets'),
        afc = S2('af_calls');
      const pos2 = {};
      rws.forEach(r => {
        const pj = r.pos_json || {};
        Object.keys(pj).forEach(k => {
          ...
          pos2[k].v += num(pj[k].v);
          pos2[k].p += num(pj[k].p);
          pos2[k].net += num(pj[k].net);
          pos2[k].hn += num(pj[k].hn);
        });
      });
      const insights = hhInsights({
        hands: hands2,
        ...
        f3bOpp: S2('f3b_opp'),
        steal: S2('steal_cnt'),
        stealOpp: S2('steal_opp'),
        bbdef: S2('bbdef_cnt'),
        bbdefOpp: S2('bbdef_opp'),
        cbet: S2('cbet_cnt'),
        cbetOpp: S2('cbet_opp'),
        fcb: S2('fcbet_cnt'),
        fcbOpp: S2('fcbet_opp'),
        ...
        wsd: S2('wsd_cnt'),
        wwsf: S2('wwsf_cnt'),
        sawflop: S2('sawflop_cnt'),
        afB: afb,
        afC: afc,
        sorte,
        allinCnt: S2('allin_cnt'),
        pos: pos2,
        bb100: hands2 > 0 ? S2('net_bb') / hands2 * 100 : 0
      });
      ...
      const MET = [['VPIP', S2('vpip_cnt'), hands2, HH_BANDS.vpip], ['PFR', S2('pfr_cnt'), hands2, HH_BANDS.pfr], ['3-bet', S2('tb_cnt'), S2('tb_opp'), HH_BANDS.tb], ['Fold pra 3-bet', S2('f3b_cnt'), S2('f3b_opp'), HH_BANDS.f3b], ['Roubo de blinds', S2('steal_cnt'), S2('steal_opp'), HH_BANDS.steal], ['BB defende', S2('bbdef_cnt'), S2('bbdef_opp'), HH_BANDS.bbdef], ['C-bet flop', S2('cbet_cnt'), S2('cbet_opp'), HH_BANDS.cbet], ['Fold pra c-bet', S2('fcbet_cnt'), S2('fcbet_opp'), HH_BANDS.fcbet], ['WWSF', S2('wwsf_cnt'), S2('sawflop_cnt'), HH_BANDS.wwsf], ['WTSD', S2('wtsd_cnt'), S2('sawflop_cnt'), HH_BANDS.wtsd], ['W$SD', S2('wsd_cnt'), S2('wtsd_cnt'), HH_BANDS.wsd]];
      const stOf = (c, o, b) => {
        if (!o) return ['sem amostra', '#6B6455'];
        const pv = c / o * 100;
        if (pv >= b[0] && pv <= b[1]) return ['dentro', C.greenMid];
        if (pv >= b[0] - 5 && pv <= b[1] + 5) return ['atenção', C.gold];
        return ['fora', C.red];
      };
      const TONE2 = {
        red: C.red,
      ...
      corpo = /*#__PURE__*/React.createElement(React.Fragment, null, !solo && /*#__PURE__*/React.createElement(Sec, {
        t: "Na pool \u2014 hist\xF3ria completa"
      }, /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'grid',
          gridTemplateColumns: 'repeat(3,1fr)',
        ...
        label: "Na pool desde",
        value: pDias[0] ? `${dLabel(pDias[0])}/${pDias[0].slice(0, 4)}` : '—'
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Dias jogados",
        value: String(pDias.length)
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Semanas",
        value: String(pWeeks.length)
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Torneios (entradas)",
        value: `${pT.length} (${pEntr})`
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Investido",
        value: fmt(pInv)
      }), /*#__PURE__*/React.createElement(Big, {
        ...
        label: "Resultado na pool",
        value: fmt(pRes),
        tone: pRes >= 0 ? C.greenMid : C.red
      }), /*#__PURE__*/React.createElement(Big, {
        label: "ROI",
        value: pInv > 0 ? pctFmt(pRes / pInv * 100) : '—',
        tone: pRes >= 0 ? C.greenMid : C.red
      }), /*#__PURE__*/React.createElement(Big, {
        label: "ABI m\xE9dio",
        ...
        label: "Make-up atual",
        value: fmt(curMakeUp[sp2]),
        tone: num(curMakeUp[sp2]) > 0 ? C.red : C.greenMid
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Lucro dividido (dele)",
        value: fmt(pParteJog)
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Saque autorizado (acum.)",
        value: fmt(pSaqueAut)
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Sacado",
        value: fmt(pPago)
      ...
      }, "Melhor dia: ", /*#__PURE__*/React.createElement("b", null, pBest ? `${dLabel(pBest[0])} (${fmt(pBest[1])})` : '—'), " \xB7 Pior dia: ", /*#__PURE__*/React.createElement("b", null, pWorst ? `${dLabel(pWorst[0])} (${fmt(pWorst[1])})` : '—'), " \xB7 Maior premia\xE7\xE3o: ", /*#__PURE__*/React.createElement("b", null, pMaior && num(pMaior.prize) > 0 ? `${pMaior.tournament_name || 'torneio'} (${fmt(pMaior.prize)})` : '—'))), !solo && /*#__PURE__*/React.createElement(Sec, {
      ...
      }, fmt(w.resultado)), /*#__PURE__*/React.createElement("td", null, fmt(w.makeAnterior), " \u2192 ", fmt(w.makeFinal)), /*#__PURE__*/React.createElement("td", null, fmt(w.parteJog)), /*#__PURE__*/React.createElement("td", null, fmt(w.saqueAut)), /*#__PURE__*/React.createElement("td", null, fmt(valorSacadoFor(w.week, sp2))))))) : /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 12,
          color: '#6B6455'
        }
      }, "Sem semanas fechadas ainda.")), /*#__PURE__*/React.createElement(Sec, {
        t: `Estatísticas de jogo — hand histories (${repPeriodo})`
      }, /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'grid',
          gridTemplateColumns: 'repeat(3,1fr)',
        ...
        label: "Torneios",
        value: String(rws.length)
      }), /*#__PURE__*/React.createElement(Big, {
        label: "bb/100 (fichas)",
        value: fmtBB(bb100v),
        tone: bb100v >= 0 ? C.greenMid : C.red
      }), /*#__PURE__*/React.createElement(Big, {
        label: "All-ins",
        value: String(S2('allin_cnt'))
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Sorte (real \u2212 EV)",
        value: fmtBB(sorte),
        tone: sorte >= 0 ? C.greenMid : C.red
      }), /*#__PURE__*/React.createElement(Big, {
        label: "Agress\xE3o (AF)",
        value: afc > 0 ? (afb / afc).toFixed(1).replace('.', ',') : '—'
      })), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 11,
          color: '#6B6455',
      ...
      }, "Sorte: real ", fmtBB(netbb), " vs esperado pela equity ", fmtBB(evbb), " nos all-ins \u2014 a diferen\xE7a \xE9 vari\xE2ncia, n\xE3o m\xE9rito ou erro.")), /*#__PURE__*/React.createElement(Sec, {
        t: "Estat\xEDsticas vs faixa de reg (MTT micro/low)"
      }, /*#__PURE__*/React.createElement("table", {
        className: "reptable"
      }, /*#__PURE__*/React.createElement("thead", null, /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("th", null, "Estat\xEDstica"), /*#__PURE__*/React.createElement("th", null, "Valor"), /*#__PURE__*/React.createElement("th", null, "Amostra"), /*#__PURE__*/React.createElement("th", null, "Faixa reg"), /*#__PURE__*/React.createElement("th", null, "Status"))), /*#__PURE__*/React.createElement("tbody", null, MET.map(([nm, c, o, b]) => {
        const [lbl, cor] = stOf(c, o, b);
        return /*#__PURE__*/React.createElement("tr", {
          key: nm
        }, /*#__PURE__*/React.createElement("td", null, nm), /*#__PURE__*/React.createElement("td", {
          style: {
            fontWeight: 700
          }
        }, o > 0 ? pctFmt(c / o * 100) : '—'), /*#__PURE__*/React.createElement("td", null, c, "/", o), /*#__PURE__*/React.createElement("td", null, b[0], "\u2013", b[1], "%"), /*#__PURE__*/React.createElement("td", {
          style: {
            color: cor,
            fontWeight: 700
          }
        ...
        t: "Por posi\xE7\xE3o"
      }, ['EP', 'MP', 'CO', 'BTN', 'SB', 'BB'].some(k => pos2[k] && pos2[k].h > 0) ? /*#__PURE__*/React.createElement("table", {
        className: "reptable"
      }, /*#__PURE__*/React.createElement("thead", null, /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("th", null, "Posi\xE7\xE3o"), /*#__PURE__*/React.createElement("th", null, "M\xE3os"), /*#__PURE__*/React.createElement("th", null, "VPIP"), /*#__PURE__*/React.createElement("th", null, "PFR"), /*#__PURE__*/React.createElement("th", null, "BB/100"))), /*#__PURE__*/React.createElement("tbody", null, ['EP', 'MP', 'CO', 'BTN', 'SB', 'BB'].filter(k => pos2[k] && pos2[k].h > 0).map(k => /*#__PURE__*/React.createElement("tr", {
        key: k
      }, /*#__PURE__*/React.createElement("td", null, /*#__PURE__*/React.createElement("b", null, k)), /*#__PURE__*/React.createElement("td", null, pos2[k].h), /*#__PURE__*/React.createElement("td", null, pctFmt(pos2[k].v / pos2[k].h * 100)), /*#__PURE__*/React.createElement("td", null, pctFmt(pos2[k].p / pos2[k].h * 100)), /*#__PURE__*/React.createElement("td", null, pos2[k].hn > 0 ? fmtBB(pos2[k].net / pos2[k].hn * 100).replace(' bb', '') : '—'))))) : /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 12,
          color: '#6B6455'
        }
      }, "Sem dados de posi\xE7\xE3o.")), /*#__PURE__*/React.createElement(Sec, {
        t: "Leituras do jogo"
      }, insights.length ? insights.map((iz, ix) => /*#__PURE__*/React.createElement("div", {
        key: ix,
        style: {
          marginBottom: 8
        }
      ...
      }, iz.t), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 12,
          color: '#3d463f',
          lineHeight: 1.5
        }
      }, iz.x))) : /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 12,
          color: '#6B6455'
        }
      }, "Sem leituras \u2014 importe hand histories.")));
    }
    return /*#__PURE__*/React.createElement("div", {
      className: "printrep",
      style: {
    ...
    }, solo ? 'GrinderBank' : 'PoolGG', " \xB7 gerado pelo app \xB7 valores financeiros v\xEAm dos lan\xE7amentos; estat\xEDsticas de jogo v\xEAm dos hand histories importados.")));
  })(), modal && (() => {
    const type = typeof modal === 'string' ? modal : modal.type;
    const initial = typeof modal === 'object' ? modal.initial : null;
    if (type === 'transfer') return /*#__PURE__*/React.createElement(AddModal, {
      title: "Nova transfer\xEAncia",
      fields: FIELDS.transfer,
      initial: initial,
      onClose: () => setModal(null),
      onSave: addTransfer
    });
    const [list, setter] = MODAL_STATE[type];
    return /*#__PURE__*/React.createElement(AddModal, {
      title: `Novo ${MODAL_TITLE[type]}`,
      fields: FIELDS[type],
      initial: initial,
      onClose: () => setModal(null),
    ...
    const [list, setter] = MODAL_STATE[editing.type];
    return /*#__PURE__*/React.createElement(AddModal, {
      title: `Editar ${MODAL_TITLE[editing.type]}`,
      fields: FIELDS[editing.type],
      editing: editing.item,
      onClose: () => setEditing(null),
      onEdit: d => update(MODAL_TABLE[editing.type], normalizeRow(editing.type, d), setter, list)
    });
  })(), quickEdit && /*#__PURE__*/React.createElement(QuickEditModal, {
    label: quickEdit.label,
    hint: quickEdit.hint,
    kind: quickEdit.kind,
    currentValue: quickEdit.current,
    onClose: () => setQuickEdit(null),
    onSave: quickEdit.onSave
  }), changePass && /*#__PURE__*/React.createElement(ChangePassModal, {
    ...
    s: 20
  })), "Fora da grade"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setAlertDetail(null),
    style: {
      width: 38,
      height: 38,
  ...
  }, "Torneios desta semana com buy-in acima do ABI m\xE1ximo do jogador. Toque no l\xE1pis pra corrigir."), sortByCreated(alertDetail.items || []).map(t => /*#__PURE__*/React.createElement(Row, {
    key: t.id,
    onEdit: () => {
      setAlertDetail(null);
      editTour(t);
    ...
    left: /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
      ...
      style: {
        color: PLAYER_COLORS[players.indexOf(t.player)] || C.inkSoft,
        fontWeight: 700
      }
    }, "\xB7 ", t.player)), /*#__PURE__*/React.createElement("div", {
      style: {
    ...
    }, dLabel(t.entry_date), " \xB7 buy-in ", fmt(t.buyin), " ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.red,
        fontWeight: 700
      }
    }, "(m\xE1x ", fmt(abiMaxFor(config, t.player, t.entry_date)), ")")))),
    right: /*#__PURE__*/React.createElement("span", {
      style: {
        fontWeight: 800,
        fontSize: 15,
        color: lucroTorneio(t) >= 0 ? C.greenMid : C.red,
        flexShrink: 0
      }
    }, lucroTorneio(t) >= 0 ? '+' : '−', fmt(Math.abs(lucroTorneio(t))))
  })), (!alertDetail.items || !alertDetail.items.length) && /*#__PURE__*/React.createElement(Empty, null, "Tudo certo agora \u2014 nenhum torneio fora da grade."))), gradeWarn && /*#__PURE__*/React.createElement("div", {
  ...
  }, "Entendi"))), cfgConfirm && /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'fixed',
      inset: 0,
      background: 'rgba(20,18,30,.55)',
  ...
  }, "Confirmar altera\xE7\xE3o"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: 700,
      fontSize: 14.5,
      marginBottom: 6
    }
  }, cfgConfirm.row.label), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 10,
  ...
  }, cfgConfirm.oldShow), /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.inkSoft
    }
  }, "\u2192"), /*#__PURE__*/React.createElement("span", {
  ...
  }, cfgConfirm.newShow)), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 12.5,
      color: C.inkSoft,
      lineHeight: 1.5,
  ...
  }, "A mudan\xE7a fica registrada no ", /*#__PURE__*/React.createElement("b", null, "hist\xF3rico de altera\xE7\xF5es"), " e o outro jogador recebe um aviso na hora. Torneios antigos continuam sendo julgados pela grade da \xE9poca."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 10
    }
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setCfgConfirm(null),
    style: {
      flex: 1,
      padding: '13px 0',
      borderRadius: 13,
  ...
  }, "Confirmar")))), askTour && /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'fixed',
      inset: 0,
      background: 'rgba(20,18,30,.55)',
  ...
  }, "Quer um tour r\xE1pido?"), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 13.5,
      color: C.inkSoft,
      lineHeight: 1.6,
  ...
  }, "Em 1 minuto eu te mostro cada tela e o que ela faz. D\xE1 pra rever depois nos Ajustes."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 10
    }
    ...
    onClick: () => {
      setAskTour(false);
      localStorage.setItem('gb_tour', '1');
    },
    style: {
      flex: 1,
      padding: '13px 0',
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        width: '100%',
        border: `1.5px solid ${P}`,
        boxShadow: '0 14px 40px -14px rgba(76,62,146,.5)'
      }
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
    ...
    }, "TOUR \xB7 ", tour, " de ", TOUR_STEPS.length), /*#__PURE__*/React.createElement("button", {
      onClick: fim,
      style: {
        marginLeft: 'auto',
        border: 'none',
    ...
    }, "Pular tour")), /*#__PURE__*/React.createElement("div", {
      style: {
        fontFamily: "'Space Grotesk',sans-serif",
        fontSize: 16.5,
        fontWeight: 700
    ...
    }, st.x), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 8,
        alignItems: 'center'
      ...
      style: {
        ...
        height: 7,
        borderRadius: 99,
        background: d <= i ? P : C.border
      }
    }))), i > 0 && /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        setTour(tour - 1);
        setView(TOUR_STEPS[i - 1].v);
      },
      style: {
        padding: '10px 16px',
        borderRadius: 11,
    ...
    style: {
      ...
      maxWidth: 440,
      background: C.surface,
      borderLeft: `4px solid ${t.tone || C.red}`,
      borderRadius: 14,
...
function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const {
        data
      } = await sb.from('player_profiles').select('*').eq('user_id', session.user.id).maybeSingle();
      if (!on) return;
      // se o e-mail foi trocado no painel do Supabase, atualiza o espelho usado pelo login
      if (data && data.email !== session.user.email) sb.from('player_profiles').update({
        email: session.user.email
      ...
      if (!data) {
        let pend = null;
        try {
          pend = JSON.parse(localStorage.getItem('gb_signup') || 'null');
        } catch (e) {}
        if (pend && pend.nickname) {
          const {
            ...
            user_id: session.user.id,
            email: session.user.email,
            nickname: pend.nickname,
            whatsapp: pend.whatsapp || null,
            password_changed: true,
            role: 'solo'
          }).select().single();
          if (!pe && novo) {
  ...
  if (!CONFIGURED) return /*#__PURE__*/React.createElement(NotConfigured, null);
  // veio do link "esqueci minha senha": trava TUDO até criar a senha nova (sem fechar, sem pular)
  if (recovery) return /*#__PURE__*/React.createElement(RecoveryReset, null);
  if (session === undefined) return /*#__PURE__*/React.createElement("div", {
    style: {
      minHeight: '100vh',
      display: 'grid',
      placeItems: 'center'
  ...
  if (profile === undefined) return /*#__PURE__*/React.createElement("div", {
    style: {
      minHeight: '100vh',
      display: 'grid',
      placeItems: 'center'
  ...
  if (!profile || !profile.password_changed) return /*#__PURE__*/React.createElement(Onboarding, {
    session: session,
    profile: profile,
    onDone: setProfile
  });
